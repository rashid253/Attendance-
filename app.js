window.addEventListener('DOMContentLoaded', async () => {
  ...
  function updateCounters() {
    const cl = $('teacherClassSelect').value, sec = $('teacherSectionSelect').value;
    $('sectionCount').dataset.target = students.filter(s=>s.cls===cl&&s.sec===
sec).length;
  ...
  // --- 7. STUDENT REGISTRATION & FINE/STATUS & 8. PAYMENT & 9. MARK ATTENDANCE & 11. ATTENDANCE REGISTER ---
  // (unchanged – your existing logic goes here)

  // --- 10. ANALYTICS PDF & SHARE ---
  $('downloadAnalytics').onclick = async () => {
    if (!lastAnalyticsStats.length) { alert('No analytics to download. Generate report first.'); return; }

    // buildHeader helper for each page
    const buildHeader = doc => {
      ...
      doc.setFontSize(10);
      doc.text(`Date: ${today}`, pageWidth - 14, 16, { align: 'right' });
      doc.text(`Period: ${lastAnalyticsRange.from} to ${lastAnalyticsRange.to}`, 14, 34);
      doc.setFontSize(12);
    };

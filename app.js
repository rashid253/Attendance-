...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  async function loadSetup() {
    ...
    if (school && cls && sec) {
      ...
      renderStudents(); updateCounters(); resetAllViews();
    }
  }
  ...
  // clear attendance, analytics, register when class/section changes
  function resetAllViews(){
    hide($('attendanceBody'),$('attendanceSummary'),$('saveAttendance'),$('resetAttendance'),$('downloadAttendancePDF'),$('shareAttendanceSummary'));
    hide($('instructions'),$('analyticsContainer'),$('graphs'),$('analyticsActions'));
    hide($('registerTableWrapper'),$('changeRegister'),$('saveRegister'),$('downloadRegister'),$('shareRegister'));
  }
  ...
  function renderStudents(){
    ...
    students.forEach((stu, idx)=>{
      ...
      tr.innerHTML=`<td><input type="checkbox" class="sel"></td><td>${++count}</td><td>${stu.name}</td><td>${stu.adm}</td><td>${stu.parent}</td><td>${stu.contact}</td><td>${stu.occupation}</td><td>${stu.address}</td><td>${$('shareRegistration').classList.contains('hidden')?'':`<i class="fas fa-share-alt share-row" data-index="${idx}"></i>`}</td>`;
  ...
  $('addStudent').onclick=async e=>{e.preventDefault(); const name=$('studentName').value.trim(),parent=$('parentName').value.trim(),contact=$('parentContact').value.trim(),occupation=$('parentOccupation').value.trim(),address=$('parentAddress').value.trim(),cls=$('teacherClassSelect').value,sec=$('teacherSectionSelect').value;
    ...
    const adm=await generateAdmNo();students.push({name,adm,parent,contact,occupation,address,cls,sec});await saveStudents();renderStudents();updateCounters();resetAllViews();
    ['studentName','parentName','parentContact','parentOccupation','parentAddress'].forEach(id=>$(id).value='');
  };
  $('deleteSelected').onclick=async()=>{if(!confirm('Delete selected?'))return; const toDel=Array.from(document.querySelectorAll('.sel:checked')).map(cb=>+cb.closest('tr').dataset.index);
    students=students.filter((_,i)=>!toDel.includes(i)); await saveStudents(); renderStudents(); updateCounters(); resetAllViews();
  };

  // ... the rest of student edit/share/download handlers unchanged ...
  ...
  saveAttendance.onclick=async()=>{
    ...
    const tbl=document.createElement('table');tbl.innerHTML='<tr><th>Name</th><th>Status</th></tr>'+filtered.map(s=>`<tr><td>${s.name}</td><td>${statusNames[attendanceData[date][s.adm]]}</td></tr>`).join('');attendanceSummary.innerHTML=`<h3>Attendance Report: ${date}</h3>`;attendanceSummary.appendChild(tbl);
  ...
  loadAnalyticsBtn.onclick=()=>{
    ...
    const cls=$('teacherClassSelect').value,sec=$('teacherSectionSelect').value,base=students.filter(s=>s.cls===cls&&s.sec===sec);
    ...
    const stats=pool.map(s=>({adm:s.adm,name:s.name,P:0,A:0,Lt:0,HD:0,L:0,total:0}));Object.entries(attendanceData).forEach(([d,recs])=>{if(d<from||d>to)return;stats.forEach(st=>{const c=recs[st.adm]||'A';st[c]++;st.total++;});});
  ...
  loadRegisterBtn.onclick=()=>{
    ...
    const cls=$('teacherClassSelect').value,sec=$('teacherSectionSelect').value,filtered=students.filter(s=>s.cls===cls&&s.sec===sec);

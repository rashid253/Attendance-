...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  loadA.onclick = () => {
    ...
    if (atype.value === 'date') {
      ...
      from = `${ayear.value}-${amonth.value.padStart(2, '0')}-01`;
      to = `${ayear.value}-${amonth.value.padStart(2, '0')}-${new Date(ayear.value, amonth.value, 0).getDate()}`;
    ...
    // Assuming you will have the logic to fetch analytics data here...

    // New functionality to download analytics PDF with graphs
    $('downloadAnalyticsPDF').onclick = () => {
      ...
      // Title for Bar Graph
      doc.text('Bar Chart', 14, 34);
      const barImageData = barCtx.canvas.toDataURL('image/png');
      doc.addImage(barImageData, 'PNG', 14, 40, 180, 90); // Adjust width and height to fit
      
      // Title for Pie Graph
      doc.text('Pie Chart', 14, 140);
      const pieImageData = pieCtx.canvas.toDataURL('image/png');
      doc.addImage(pieImageData, 'PNG', 14, 150, 180, 90); // Adjust width and height to fit

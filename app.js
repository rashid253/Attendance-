...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('saveAttendance').onclick = async () => {
    ...
    summary.innerHTML = `<h3>Attendance Report: ${date}</h3><table><tr><th>Name</th><th>Status</th><th>Share</th></tr>` +
    ...
    summary.querySelectorAll('.share-individual').forEach(ic=>ic.onclick=()=>{
      ...
      const msg = `Dear Parent, your child was ${{'P':'Present','A':'Absent','Lt':'Late','HD':'Half-Day','L':'Leave'}[code]} on ${date}.`;
  ...
  const atg = $('analyticsTarget'), asel = $('analyticsSectionSelect'), atype = $('analyticsType'),
        ...
        filterDialog   = $('analyticsFilterDialog'),
  ...
  closeDialog.addEventListener('click', () => filterDialog.close());
  ...
  function applyAnalyticsFilters() {
    ...
    document.querySelectorAll('#analyticsBody tr').forEach(row=>{
      ...
      const out = parseFloat(row.cells[10].textContent.replace(/[^0-9\.]/g,''));
  ...
  loadA.onclick = () => {
    ...
    stats.forEach((st,i)=>{
      const pct = st.total?((st.P/st.total)*100).toFixed
      (1):'0.0';
    ...
    show(instr, acont, graphs, downloadReports);

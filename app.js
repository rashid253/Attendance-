...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // --- IndexedDB via idb-keyval IIFE ---
  if (!window.idbKeyval) {
    console.error('idbKeyval not found. Did you include the iife script?');
    return;
  }
  ...
  saveAttendance.onclick = async () => {
    ...
    attendanceSummary.innerHTML = `<h3>Date: ${date}</h3><ul>` +
  ...
  function hideAllAnalytics() {
    [analyticsDate, analyticsMonth, semesterStartInput, semesterEndInput,
     ...
     analyticsActionsEl, resetAnalyticsBtn].forEach(hide);
  }
  ...
  resetAnalyticsBtn.onclick = e => { e.preventDefault(); hideAllAnalytics(); analyticsType.value=''; };
  loadAnalyticsBtn.onclick = () => {
    ...
    if (tgt==='student') pool=pool.filter(s=>s.adm===analyticsStudent.value);
    ...
    show(instructionsEl); show(analyticsContainer); show(graphsEl); show(analyticsActionsEl);
  ...
  $('downloadAnalytics').onclick = () => {
    ...
    d.autoTable({ html:'#analyticsTable' }); d.save('analytics.pdf');
  };

...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('downloadAnalytics').onclick = async () => {
    ...
    if (analyticsDownloadMode === 'combined') {
      ...
      // Individual mode: single PDF with one page per student
      const doc = new jspdf.jsPDF();
      lastAnalyticsStats.forEach((stat, i) => {
        if (i > 0) doc.addPage();
        doc.setFontSize(18).text(`Analytics: ${stat.name}`, 14, 16);
        doc.setFontSize(12)
           .text(`Adm#: ${stat.adm}`, 14, 24)
           .text(`Period: ${lastAnalyticsRange.from} to ${lastAnalyticsRange.to}`, 14, 32);
        doc.autoTable({
          startY:  Forty,  // ← use a number, e.g.  Forty = 40
      ...
      const blob = doc.output('blob');
      doc.save(fileName);
      await sharePdf(blob, fileName, 'Individual Analytics Reports');
    }
  };
  ...
  loadAttendanceBtn.onclick = () => {
    ...
    students.filter(s=>s.cls===cl&&s.sec===sec).forEach((stu,i)=>{
      ...
      Object.entries(statusNames).forEach(([code])=>{
  ...
  saveAttendanceBtn.onclick = async () => {
    ...
    students.filter(s=>s.cls===cl&&s.sec===sec).forEach(s=>{
      ...
      tbl.innerHTML+=`
        <tr>
          <td>${s.name}</td><td>${statusNames[code]}</td>
    ...
    students.filter(s=>s.cls===cl&&s.sec===sec).forEach((s,i)=>{
      ...
      for (let d=1; d<=days; d++){
        const key=`${m}-${String(d).padStart(2,'0')}`,
              ...
              style = c==='A'?'':`style="background:${color};color:#fff"`;
    ...
    document.querySelectorAll('.reg-cell').forEach(cell=>{
      cell.onclick=()=>{
        const span=cell.querySelector('.status-text'),
              ...
              c=codes[idx],
              color=c==='A'?'':(c==='P'?'var(--success)':c==='Lt'?'var(--warning)':c==='HD'?'#FF9800':c==='L'?'var(--info)':'var(--danger)');
        span.textContent=c;
        if (c==='A') { cell.style.background=''; cell.style.color=''; }
        else { cell.style.background=color; cell.style.color='#fff'; }
      };
    });

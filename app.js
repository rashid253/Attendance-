...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  loadAnalyticsBtn.onclick=()=>{ /* compute stats... */ hide(analyticsSectionSel,analyticsType,analyticsDate,analyticsMonth,semesterStart,semesterEnd,yearStart,analyticsSearch); show(instructionsEl,analyticsContainer,graphsEl,analyticsActions); };
  ...
  loadRegisterBtn.onclick=()=>{ /* build table... */ hide(monthInput);show(saveRegisterBtn);show(changeRegisterBtn); };
  saveRegisterBtn.onclick=async()=>{/* save... */hide(saveRegisterBtn);show(downloadRegister,shareRegister);registerWrapper.prepend(Object.assign(document.createElement('h3'),{textContent:`Month: ${monthInput.value}`}));};

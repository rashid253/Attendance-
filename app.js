...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('saveSetup').onclick = async e => {
    ...
    await Promise.all([
      set('schoolName', school), set('teacherClass', cls), set('teacherSection', sec)
    ]);
    await loadSetup();
  };
  ...
  function renderStudents() {
    ...
    students.filter(s=>s.cls===cls&&s.sec===sec).forEach((stu,i) => {
      ...
      `;
      // share icon cell
      rowHTML += `<td>${isShareMode? `<i class="fas fa-share-alt share-row" data-index="${i}"></i>` : ''}</td>`;
      tr.innerHTML = rowHTML;
      tbody.appendChild(tr);
    });
    $('selectAllStudents').checked = false;
    toggleSelectionButtons();
    // attach share-row listeners
    if (isShareMode) {
...
Address: ${s.address}`;
  ...
  $('editRegistration').onclick = e => {
    e.preventDefault();
    isShareMode = false;
    show($('editSelected'), $('deleteSelected'), $('saveRegistration'), $('selectAllStudents'));
    hide($('shareRegistration'), $('downloadRegistrationPDF'));
    renderStudents();
  };
    ...
    students.forEach(stu => {
      ...
      Object.entries(statusNames).forEach(([code, full]) => {
        ...
        btn.onclick = () => {
          ...
          btn.style.background = `var(--${full.toLowerCase().replace(' ','')})`; // ensure CSS var keys
          btn.style.color = '#fff';
        };
  ...
  // ... download & share summary, reset handlers remain same as before ...

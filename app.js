...
$('shareAnalytics').onclick = () => {
  ...
  // 1) Prompt once for a backup folder and remember it
  ...
  // 3) Write (overwrite) the single backup file
  async function writeBackupFile(data) {
    const dir        = await getBackupDirHandle();
    const handle     = await dir.getFileHandle('attendance-backup.json', { create: true });
    const writable   = await handle.createWritable();
    await writable.write(JSON.stringify(data, null, 2));
    await writable.close();
    console.log('✅ Backup written at', new Date().toLocaleTimeString());
  }

  // 4) Restore app state from that file
  ...
  // 5) Auto-restore on startup if local DB is empty
  const studentsData = await get('students');
  if (!studentsData || studentsData.length === 0) {
    try {
      const dir      = await getBackupDirHandle();
      const file     = await (await dir.getFileHandle('attendance-backup.json')).getFile();
  ...
  // 6) Schedule an auto-backup every 5 minutes
  setInterval(async () => {
    try {

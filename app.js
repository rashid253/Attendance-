...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('editSelected').onclick = e => {
    ...
    $('editSelected').onclick = evt => {
      ...
      $('editSelected').onclick = /* this same function */;
    };
  };
  ...
  const resetAttendance       = $('resetAttendance');
  const downloadAttendancePDF = $('downloadAttendancePDF');
  const shareAttendanceSummary= $('shareAttendanceSummary');
  ...
  function hideAllAnalytics() {
    [analyticsDate, analyticsMonth, semesterStartInput,
     semesterEndInput, yearStartInput, instructionsEl,
     analyticsContainer, graphsEl, analyticsActionsEl,
     resetAnalyticsBtn].forEach(el => hide(el));
  }
  ...
  analyticsType.onchange = () => {
    ...
    if (analyticsType.value === 'semester') {
      show(semesterStartInput);
      show(semesterEndInput);
    }

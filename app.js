...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  function resetViews(){
    hide($('attendanceBody'),$('saveAttendance'),$('resetAttendance'),
         ...
         $('downloadRegister'),$('shareRegister'),$('downloadRegistrationPDF'),$('shareRegistration'));
    show($('loadRegister'));
  }
  ...
  function renderStudents(){
    ...
    students.forEach((s,i)=>{
      ...
      const paid=(paymentsData[s.adm]||[]).reduce((a,p)=>a+p.amount,0), outstanding=totalFine-paid;
  ...
  $('saveRegistration').onclick=async()=>{
    ...
    await save('students',students);
    hide(...document.querySelectorAll('#student-registration .row-inline, #selectAllStudents, #editSelected, #deleteSelected, #saveRegistration'));
    show($('editRegistration'),$('shareRegistration'),$('downloadRegistrationPDF'));
    renderStudents(); updateCounters();
  };
  $('editRegistration').onclick=()=>{
    show(...document.querySelectorAll('#student-registration .row-inline, #selectAllStudents, #editSelected, #deleteSelected, #saveRegistration'));
    hide($('editRegistration'),$('shareRegistration'),$('downloadRegistrationPDF'));
    renderStudents(); updateCounters();
  };

  // student registration download/share
  $('downloadRegistrationPDF').onclick=async()=>{
    const doc=new jspdf.jsPDF();
    doc.setFontSize(18); doc.text('Student Registration List',14,16);
    doc.setFontSize(12); doc.text($('setupText').textContent + ' | ' + new Date().toLocaleDateString(),14,24);
    doc.autoTable({ startY:40, html:'#student-registration table', styles:{ fontSize:10 } });
    const blob=doc.output('blob'); doc.save('students.pdf');
    await sharePdf(blob,'students.pdf','Student List');
  };
  $('shareRegistration').onclick=()=>{
    const header=`Student List\n${$('setupText').textContent}`;
    const rows=[...$('studentsBody').children].map(tr=>[tr.children[2].textContent,tr.children[3].textContent].join(' '));
  ...
  downloadAttendanceBtn.onclick=async()=>{
    const date=dateInput.value; const doc=new jspdf.jsPDF();
    doc.setFontSize(18); doc.text('Attendance Report',14,16);
    doc.setFontSize(12); doc.text($('setupText').textContent + ' | ' + date,14,24);
    doc.autoTable({ startY:40, html:'#attendanceSummary table' });
    const blob=doc.output('blob'); doc.save(`attendance_${date}.pdf`); await sharePdf(blob,`attendance_${date}.pdf`,'Attendance Report');
  };
  ...
  loadA.onclick=()=>{
    ...
    stats.forEach(st=>{
      ...
      st.outstanding=tf-tp; const pct=st.total?st.P/st.total*100:0;
  ...
  function renderAnalytics(stats,from,to){
    ...
    barChart=new Chart(barCtx,{type:'bar',data:{
      ...
      datasets:statusKeys.map(code=>({label:statusNames[code],data:filtered.map(st=>st.total?st[code]/st.total*100:0),backgroundColor:statusColors[code]}))
    },options:{scales:{x:{stacked:true},y:{stacked:true,beginAtZero:true,max:100,title:{display:true,text:'% of Days'}}},plugins:{tooltip:{mode:'index',intersect:false},legend:{position:'bottom'}},responsive:true,interaction:{mode:'index',intersect:false}}});

    // pie
    pieChart?.destroy();
    const totals=statusKeys.map(code=>filtered.reduce((sum,st)=>sum+st[code],0));
    pieChart=new Chart(pieCtx,{type:'pie',data:{labels:statusKeys.map(c=>statusNames[c]),datasets:[{data:totals,backgroundColor:statusKeys.map(c=>statusColors[c])}]},options:{plugins:{legend:{position:'right'}},responsive:true}});
  ...
  $('downloadAnalytics').onclick=async()=>{
    const [from,to]=[lastAnalyticsRange.from,lastAnalyticsRange.to];
    const doc=new jspdf.jsPDF();
    doc.setFontSize(18); doc.text('Analytics Report',14,16);
    doc.setFontSize(12); doc.text($('setupText').textContent + ` | ${from} to ${to}`,14,24);
    doc.autoTable({ startY:40, html:'#analyticsTable', styles:{ fontSize:10 } });
    const blob=doc.output('blob'); doc.save('analytics.pdf'); await sharePdf(blob,'analytics.pdf','Analytics Report');
  };
  ...
  $('loadRegister').onclick=()=>{
    ...
    students.filter(s=>s.cls===cl&&s.sec===sec).forEach((s,i)=>{
      ...
      for(let d=1;d<=days;d++){
        ...
        const style=c==='A'?'':`style="background:${
          c==='P'?'var(--success)':c==='Lt'?'var(--warning)':c==='HD?'#FF9800':c==='L'?'var(--info)':'var(--danger)'
        };color:#fff"`;
    ...
    document.querySelectorAll('.reg-cell').forEach(cell=>cell.onclick=()=>{
      const span=cell.querySelector('.status-text'), idx=(['A','P','Lt','HD','L'].indexOf(span.textContent)+1)%5, c=['A','P','Lt','HD','L'][idx];
      span.textContent=c; if(c==='A'){cell.style.background='';cell.style.color='';}else{const color=c==='P'?'var(--success)':c==='Lt'?'var(--warning)':c==='HD'?'#FF9800':c==='L'?'var(--info)':'var(--danger)';cell.style.background=color;cell.style.color='#fff';}
    });
  ...
  $('changeRegister').onclick=()=>{ hide($('registerTableWrapper'),$('changeRegister'),$('downloadRegister'),$('shareRegister'),$('saveRegister')); $('registerHeader').innerHTML=''; $('registerBody').innerHTML=''; show($('loadRegister')); };

  $('downloadRegister').onclick=async()=>{
    const m=$('registerMonth').value; const doc=new jspdf.jsPDF({orientation:'landscape',unit:'pt',format:'a4'});
    doc.setFontSize(18); doc.text('Attendance Register',14,16);
    doc.setFontSize(12); doc.text($('setupText').textContent + ' | ' + m,14,24);
    doc.autoTable({ startY:40, html:'#registerTable', styles:{fontSize:10} });
  ...
  $('shareRegister').onclick=()=>{
    const header=`Attendance Register\n${$('setupText').textContent}`; const rows=[...$('registerBody').children].map(tr=>[...tr.children].map(td=>td.querySelector('.status-text')?.textContent||td.textContent).join(' '));
    window.open(`https://wa.me/?text=${encodeURIComponent(header+'\n'+rows.join('\n'))}`,'_blank');
  };

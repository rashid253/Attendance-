...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // --- 1. Student List PDF Download & Share ---
  $('downloadRegistrationPDF').onclick = async () => {
    ...
    doc.text(`Date: ${date}`, width - 14, 24, { align: 'right' });
  ...
  // --- 2. Attendance Summary PDF Download & Share ---
  ...
  downloadAttendanceBtn.onclick = async () => {
    if (!$('#attendanceSummary').innerHTML.trim()) {
      alert('No attendance to download. Generate a report first.');
      return;
    }
    const doc = new jspdf.jsPDF();
    const width = doc.internal.pageSize.getWidth();
  ...
  shareAttendanceBtn.onclick = () => {
    ...
    const lines = Array.from($('#attendanceSummary table tbody tr')).map(tr => {
  ...
  // --- 3. Attendance Register PDF Download & Share ---
  $('downloadRegister').onclick = async () => {
    ...
    doc.text(`Date: ${date}`, width - 14, 36, { align: 'right' });
  ...
  $('loadAttendance').onclick=()=>{
    const attendanceBodyDiv = $('attendanceBody'), attendanceSummaryDiv = $('attendanceSummary'),
    ...
    students.filter(s=>s.cls===cl&&s.sec===sec).forEach((stu,i)=>{
      ...
      ['P','A','Lt','HD','L'].forEach(code=>{
        ...
        btn.onclick=()=>{
          ...
          btn.classList.add('selected'); btn.style.background=getComputedStyle(document.documentElement).getPropertyValue(`--${code==='Lt'?'warning':code==='HD'?'orange':code==='L'?'info':code==='A'?'danger':'success'}`).trim();
          btn.style.color='#fff';
        };
    ...
    hide(resetAttendanceBtn, downloadAttendanceBtn, shareAttendanceBtn, $('attendanceSummary'));
  };
  ...
  $('saveAttendance').onclick=async ()=>{
    ...
    const summary = $('attendanceSummary'); summary.innerHTML = `<h3>Attendance Report: ${date}</h3><table><tr><th>Name</th><th>Status</th><th>Share</th></tr></table>`;
    const tbl = summary.querySelector('table');
    students.filter(s=>s.cls===cl&&s.sec===sec).forEach(s=>{
      ...
      const icon = document.createElement('i'); icon.className='fas fa-share-alt share-individual'; icon.dataset.adm=s.adm;
      shareCell.appendChild(icon);
    });
    summary.querySelectorAll('.share-individual').forEach(ic=>{
      ic.onclick=()=>{
        ...
        window.open(`https://wa.me/${st.contact}?text=${encodeURIComponent(`Dear Parent, your child was ${code} on ${date}.`)}`, '_blank');
      };
    });

    show(resetAttendanceBtn, $('downloadAttendancePDF'), $('shareAttendanceSummary'), summary);
  ...
  // --- 10. Analytics & Charts (Unchanged) ---
  // ... existing analytics code remains here ...
  ...
  $('loadRegister').onclick=()=>{
    ...
    const header=$('registerHeader'); header.innerHTML='<th>#</th><th>Adm#</th><th>Name</th>'+[...Array(days)].map((_,i)=>`<th>${i+1}</th>`).join('');

window.addEventListener('DOMContentLoaded', () => {
  ...
  $('shareAttendanceSummary').onclick = () => {
    const lines = [...summaryBody.querySelectorAll('tr')].map(r=>{
      const name=r.children[0].textContent, stat=r.children[1].textContent;
      return `Name: ${name}\nStatus: ${stat}`;
    }).join('\n---\n');
  ...
  function buildDates(){ const v=typeSel.value, arr=[];
    function push(s,e){ let c=new Date(s); while(c<=e){ arr.push(c.toISOString().slice(0,10)); c.setDate(c.getDate()+1); }}
  ...
  shareAnalytics.onclick = () => {
    if(!summaryData.length) return alert('No data');
  ...
  downloadAnalytics.onclick = () => {
    ...
    y=doc.lastAutoTable.finalY+20; doc.text('Bar Chart',20,y); y+=10;

...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // --- 4. FINANCIAL SETTINGS: Fines & Eligibility ---
...
// app.js — Part 2 of 2 (continuation)

// 8. Registration Editing & Saving
...
$('saveRegistration').onclick = async () => {
  ...
  await save('students', students);
  hide($('selectAllStudents'), $('editSelected'), $('deleteSelected'), $('saveRegistration'));
...
$('shareRegistration').onclick = () => {
  ...
  const lines = students.filter(s => s.cls===cl && s.sec===sec).map(s => {
    const tf = Object.values(attendanceData).reduce((sum, rec) => sum + (rec[s.adm]==='A'?fineRates.A:0), 0);
    ...
    const pct = days?((pres/days)*100).toFixed(1):0;
...
loadAttendanceBtn.onclick = () => {
  ...
  roster.forEach((stu,i) => {
    const row = document.createElement('div'), nameDiv = document.createElement('div'),
          btnsDiv = document.createElement('div');
    row.className = 'attendance-row'; nameDiv.className = 'attendance-name';
    nameDiv.textContent = stu.name; btnsDiv.className = 'attendance-buttons';
...
saveAttendanceBtn.onclick = async () => {
  ...
  roster.forEach(s => {
    const code = attendanceData[date][s.adm], cell = statusNames[code]||'Absent';
    tbl.innerHTML += `
      <tr>
        <td>${s.name}</td>
        <td>${cell}</td>
...
// 11. SHARE ALL ATTENDANCE
$('shareAllAttendance').onclick = () => {
  const date = dateInput.value;
  if (!date) { alert('Select a date'); return; }
  const cl = $('teacherClassSelect').value, sec = $('teacherSectionSelect').value;
  const roster = students.filter(s => s.cls===cl && s.sec===sec);
  roster.forEach(s => {
    ...
    const msg = `محترم والد/والدہ، آپ کے بچے ${s.name} نے ${date} کو ${statusNames[code]} درج کیا ہے۔`;
    window.open(`https://wa.me/${s.contact}?text=${encodeURIComponent(msg)}`, '_blank');
  });
};

// 12. ANALYTICS & FINE REPORT
...
atype.onchange = () => {
  ...
  instr.classList.add('hidden'); acont.classList.add('hidden');
  graphs.classList.add('hidden'); aacts.classList.add('hidden');
...
resetA.onclick = e => {
  e.preventDefault();
  atype.value=''; instr.classList.add('hidden');
  acont.classList.add('hidden'); graphs.classList.add('hidden');
  aacts.classList.add('hidden'); resetA.classList.add('hidden');
};
...
$('downloadAnalytics').onclick = () => {
  const doc=new jspdf.jsPDF();
  doc.setFontSize(18).text('Analytics Report',14,16);
  doc.setFontSize(12).text($('setupText').textContent,14,24);
...
loadReg.onclick = () => {
  ...
  roster.forEach((s,i)=>{
    ...
    rb.insertAdjacentHTML('beforeend',`<tr>${row}</tr>`);
  });
...
saveReg.onclick = async () => {
  const [y,mm] = rm.value.split('-').map(Number);
  const days = new Date(y,mm,0).getDate();
...
shReg.onclick = () => {
  ...
  window.open(`https://wa.me/?text=${encodeURIComponent(header+rows.join('\n'))}`, '_blank');
};

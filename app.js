...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  collapsibleIds.forEach(id => {
    ...
    if (!sec) return;
    const heading = sec.querySelector('h2') || sec.querySelector('h3');
    if (!heading) return;
  ...
  loadAttendanceBtn.onclick = () => {
    ...
    students.filter(s=>s.cls===cl&&s.sec===sec).forEach((stu,i)=>{
      ...
      const nameDiv = document.createElement('div'); nameDiv.className='attendance-name';
      nameDiv.textContent = `${i+1}. ${stu.name} (Adm#: ${stu.adm})`;
  ...
  saveAttendanceBtn.onclick = async () => {
    ...
    attendanceSummaryDiv.querySelectorAll('.share-individual').forEach(ic=>{
      ic.onclick = ()=>{
        ...
        const idx = [...students.filter(s=>s.cls===cl&&s.sec===sec)].findIndex(s=>s.adm===adm)+1;
        const msg = `Dear Parent, ${idx}. ${st.name} (Adm#: ${st.adm}) was ${statusNames[attendanceData[date][adm]]} on ${date}.`;
  ...
  downloadAttendanceBtn.onclick = async () => {
    ...
    doc.save(fileName); await sharePdf(blob,fileName,'Attendance Report');
  };
  ...
  loadA.onclick=()=>{
    ...
    stats.forEach(st=>{
      ...
      const paid=(paymentsData[st.adm]||[]).reduce((a,p)=>a+p.amount,0); st.outstanding=totalFine-paid;
  ...
  function renderAnalytics(stats,from,to){
    ...
    barChart = new Chart(barCtx,{
      type:'bar',
      data:{ labels:filtered.map(st=>st.name), datasets:[{ label:'% Present', data:filtered.map(st=>st.total?st.P/st.total*100:0), backgroundColor:filtered.map(_=>analyticsStatusColors.P) }] },
    ...
    pieChart = new Chart(pieCtx,{
      type:'pie',
      data:{ labels:Object.values(analyticsStatusNames), datasets:[{ data:Object.keys(analyticsStatusNames).map(c=>totals[c]), backgroundColor:Object.keys(analyticsStatusNames).map(c=>analyticsStatusColors[c]) }] }
    });
  ...
  (function(){
    ...
    loadBtn.onclick = ()=>{
      ...
      students.filter(s=>s.cls===cls&&s.sec===sec).forEach((s,i)=>{
        ...
        dateKeys.forEach(key=>{
          const c=attendanceData[key][s.adm]||'',
                color=c==='P'?'var(--success)':c==='Lt'?'var(--warning)':c==='HD'?'#FF9800':c==='L'?'var(--info)':'var(--danger)',
                style=c?`style="background:${color};color:#fff"`:'';
      ...
      show(tableWrap, saveBtn); hide(loadBtn, changeBtn, downloadBtn, shareBtn);
    };

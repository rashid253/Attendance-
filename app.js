...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  saveAttendanceBtn.onclick=async()=>{const date=dateInput.value; if(!date){alert('Pick date');return;}
    ...
    students.filter(s=>s.cls===cl&&s.sec===sec).forEach((s,i)=>{const b=attendanceBodyDiv.children[i].querySelector('.att-btn.selected');
  ...
  loadA.onclick=()=>{
    ...
    const cls=$('teacherClassSelect').value;let pool=students.filter(s=>s.cls===cls);
    ...
    Object.entries(attendanceData).forEach(([d,rec])=>{if(d<from||d>to)return;stats.forEach(st=>{if(rec[st.adm]){st[rec[st.adm]]++;st.total++;}});});
  ...
  $('applyAnalyticsFilter').onclick=()=>{analyticsFilterOptions=Array.from(document.querySelectorAll('#analyticsFilterForm input[type="checkbox"]:checked')).map(cb=>cb.value);if(!analyticsFilterOptions.length)analyticsFilterOptions=['all'];
    analyticsDownloadMode=document.querySelector('#analyticsFilterForm input[name="downloadMode"]:checked').value;hide($('analyticsFilterModal'));
    if(!lastAnalyticsStats.length){alert('Load first');}else renderAnalytics(lastAnalyticsStats,lastAnalyticsRange.from,lastAnalyticsRange.to);
  };
  function renderAnalytics(stats,from,to){
    const filters=analyticsFilterOptions;
    const filtered=filters.includes('all')?stats:stats.filter(st=>filters.some(opt=>{
      switch(opt){case'attendance':return st.total>0;case'fine':return st.A>0||st.Lt>0||st.L>0||st.HD>0;case'cleared':return st.outstanding===0;case'debarred':return st.status==='Debarred';case'eligible':return st.status==='Eligible';default:return true;}
    }));
    ...
    const tbody=$('analyticsBody');tbody.innerHTML='';filtered.forEach((st,i)=>{const pct=st.total?((st.P/st.total)*100).toFixed(1):'0.0';const tr=document.createElement('tr');

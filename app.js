...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('deleteSelected').onclick = e => {
    ...
    document.querySelectorAll('tr').forEach(tr => {
      ...
      if (!cb || !cb.checked) keep.push(students[+tr.dataset.index]);
    });
    students = keep;
    renderStudents();
  };
  ...
  loadAnalytics.onclick = () => {
    ...
    if (tgt === 'student') {
      pool = pool.filter(s => String(s.roll) === analyticsStudent.value);
    }
    ...
    stats.forEach((st,i) => {
      ...
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${i+1}</td><td>${st.name}</td><td>${st.P}</td><td>${st.A}</td><td>${st.Lt}</td><td>${st.HD}</td><td>${st.L}</td><td>${st.total}</td><td>${pct}%</td>`;
      tb.appendChild(tr);
    });
  ...
  resetAnalytics.onclick = e => {
    ...
    [analyticsDate, analyticsMonth, semesterStart, semesterEnd, yearStart].forEach(hide);
    analyticsType.value = '';
    // resetAnalytics remains visible
  };

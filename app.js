...
window.addEventListener('DOMContentLoaded', () => { const $ = id => document.getElementById(id); const colors = { P: 'var(--success)', A: 'var(--danger)', Lt: 'var(--warning)', HD: 'var(--orange)', L: 'var(--info)' };

// 1. SETUP const schoolIn = $('schoolNameInput'); const classSel = $('teacherClassSelect'); const secSel   = $('teacherSectionSelect'); const saveSetupBtn = $('saveSetup'); const setupForm    = $('setupForm'); const setupDisplay = $('setupDisplay'); const setupText    = $('setupText'); const editSetupBtn = $('editSetup');
...
// 3. ATTENDANCE MARKING let attendanceData = JSON.parse(localStorage.getItem('attendanceData') || '{}'); window.attendanceData = attendanceData; const dateInput       = $('dateInput'); const loadAttBtn      = $('loadAttendance'); const attList         = $('attendanceList'); const saveAttBtn      = $('saveAttendance'); const resSection      = $('attendance-result'); const summaryBody     = $('summaryBody'); const resetAttBtn     = $('resetAttendance'); const shareAttBtn     = $('shareAttendanceSummary'); const downloadAttPDFBtn = $('downloadAttendancePDF');
...
saveAttBtn.onclick = ev => { ev.preventDefault(); const d = dateInput.value; attendanceData[d] = {}; attList.querySelectorAll('.attendance-actions').forEach((btns,i) => { const sel = btns.querySelector('.att-btn[style*="background"]'); attendanceData[d][students[i].roll] = sel ? sel.dataset.code : 'A'; }); localStorage.setItem('attendanceData', JSON.stringify(attendanceData)); $('attendance-section').classList.add('hidden'); resSection.classList.remove('hidden'); summaryBody.innerHTML = ''; const hdr = Date: ${d}\nSchool: ${localStorage.getItem('schoolName')}\nClass: ${localStorage.getItem('teacherClass')}\nSection: ${localStorage.getItem('teacherSection')}; summaryBody.insertAdjacentHTML('beforebegin', <tr><td colspan="3"><em>${hdr}</em></td></tr>); students.forEach(s => { const code = attendanceData[d][s.roll] || 'A'; const status = {P:'Present',A:'Absent',Lt:'Late',HD:'Half Day',L:'Leave'}[code]; const tr = document.createElement('tr'); tr.innerHTML = <td>${s.name}</td><td>${status}</td><td><button class="send-btn">Send</button></td>; tr.querySelector('.send-btn').onclick = e2 => { e2.preventDefault(); const msg = ${hdr}\n\nName: ${s.name}\nStatus: ${status}; window.open(https://wa.me/${s.contact}?text=${encodeURIComponent(msg)}, '_blank'); }; summaryBody.appendChild(tr); }); };
...
shareAttBtn.onclick = ev => { ev.preventDefault(); const d = dateInput.value; const hdr = Date: ${d}\nSchool: ${localStorage.getItem('schoolName')}\nClass: ${localStorage.getItem('teacherClass')}\nSection: ${localStorage.getItem('teacherSection')}; const remarkMap = {P:'Present',A:'Absent',Lt:'Late',HD:'Half Day',L:'Leave'}; const lines = students.map(s => ${s.name}: ${remarkMap[attendanceData[d][s.roll] || 'A']}); const total = students.length; const pres  = students.reduce((sum,s) => sum + (attendanceData[d][s.roll]==='P'?1:0), 0); const pct   = total ? ((pres/total)*100).toFixed(1) : '0.0'; const clsRemark = pct==100?'Best':pct>=75?'Good':pct>=50?'Fair':'Poor'; const summary = Overall Attendance: ${pct}% | ${clsRemark}; const msg     = [hdr, '', ...lines, '', summary].join('\n'); window.open(https://wa.me/?text=${encodeURIComponent(msg)}, '_blank'); };

downloadAttPDFBtn.onclick = ev => { ev.preventDefault(); const { jsPDF } = window.jspdf; const doc = new jsPDF('p','pt','a4'); doc.autoTable({ head:[['Name','Status']], body: students.map(s => { const code = attendanceData[dateInput.value]?.[s.roll] || 'A'; const status = {P:'Present',A:'Absent',Lt:'Late',HD:'Half Day',L:'Leave'}[code]; return [s.name, status]; }), startY:40, margin:{left:40,right:40}, styles:{fontSize:10} }); doc.save('attendance_summary.pdf'); };

// 4. ANALYTICS const analyticsType      = $('analyticsType'); const analyticsDate      = $('analyticsDate'); const analyticsMonth     = $('analyticsMonth'); const semesterStart      = $('semesterStart'); const semesterEnd        = $('semesterEnd'); const yearStart          = $('yearStart'); const loadAnalyticsBtn   = $('loadAnalytics'); const resetAnalyticsBtn  = $('resetAnalytics'); const instructionsEl     = $('instructions'); const analyticsContainer = $('analyticsContainer'); const graphsEl           = $('graphs'); const analyticsActionsEl = $('analyticsActions'); const shareAnalyticsBtn  = $('shareAnalytics'); const downloadAnalyticsBtn = $('downloadAnalytics'); const barCtx             = document.getElementById('barChart').getContext('2d'); const pieCtx             = document.getElementById('pieChart').getContext('2d'); let barChart, pieChart;

function hideAllAnalytics() { [analyticsDate, analyticsMonth, semesterStart, semesterEnd, yearStart, instructionsEl, analyticsContainer, graphsEl, analyticsActionsEl, resetAnalyticsBtn] .forEach(el => el.classList.add('hidden')); }
...
resetAnalyticsBtn.onclick = ev => { ev.preventDefault(); analyticsType.value = ''; hideAllAnalytics(); };

loadAnalyticsBtn.onclick = ev => { ev.preventDefault(); let from, to; if (analyticsType.value==='date') { if(!analyticsDate.value) return alert('Pick a date'); from = to = analyticsDate.value; } else if (analyticsType.value==='month') { if(!analyticsMonth.value) return alert('Pick a month'); from = analyticsMonth.value+'-01'; to = analyticsMonth.value+'-31'; } else if (analyticsType.value==='semester') { if(!semesterStart.value||!semesterEnd.value) return alert('Pick range'); from = semesterStart.value+'-01'; to = semesterEnd.value+'-31'; } else if (analyticsType.value==='year') { if(!yearStart.value) return alert('Pick a year'); from = yearStart.value+'-01-01'; to = yearStart.value+'-12-31'; } else return;
...
shareAnalyticsBtn.onclick = ev => { ev.preventDefault(); const period = instructionsEl.textContent.replace('Report: ',''); const hdr = Date Range: ${period}\nSchool: ${localStorage.getItem('schoolName')}\nClass: ${localStorage.getItem('teacherClass')}\nSection: ${localStorage.getItem('teacherSection')}; const rows = Array.from(analyticsContainer.querySelectorAll('tbody tr')).map(r=>{ const td = r.querySelectorAll('td'); return ${td[0].textContent} P:${td[1].textContent} A:${td[2].textContent} Lt:${td[3].textContent} HD:${td[4].textContent} L:${td[5].textContent} Total:${td[6].textContent} %:${td[7].textContent}; }); window.open(https://wa.me/?text=${encodeURIComponent(hdr+'\n\n'+rows.join('\n'))}, '_blank'); };

downloadAnalyticsBtn.onclick = ev => { ev.preventDefault(); const { jsPDF } = window.jspdf; const doc = new jsPDF('p','pt','a4'); doc.setFontSize(14); doc.text(localStorage.getItem('schoolName'),40,30); doc.setFontSize(12); doc.text(Class: ${localStorage.getItem('teacherClass')} | Section: ${localStorage.getItem('teacherSection')},40,45); doc.text(instructionsEl.textContent.replace('Report: ','Period: '),40,60); doc.autoTable({ html: '#analyticsContainer table', startY:75, margin:{left:40,right:40}, styles:{fontSize:8} }); const y = doc.lastAutoTable.finalY + 10; const w = 120, h = 80; doc.addImage(barChart.toBase64Image(),'PNG', 40, y, w, h); doc.addImage(pieChart.toBase64Image(),'PNG', 40 + w + 20, y, w, h); doc.save('analytics_report.pdf'); };
...
loadRegBtn.onclick = e => { e.preventDefault(); if (!regMonthIn.value) return alert('Select month'); const attendanceData = JSON.parse(localStorage.getItem('attendanceData') || '{}'); const [yr,mo] = regMonthIn.value.split('-').map(Number); const dim = new Date(yr, mo, 0).getDate();

regBody.innerHTML = '';
regSummaryBody.innerHTML = '';

// populate daily codes
...
// summary: include roll for stats
const stats = students.map(s=>({name:s.name, roll:s.roll, P:0, A:0, Lt:0, HD:0, L:0, total:0}));
stats.forEach(st=>{
  for(let d=1; d<=dim; d++){
    const key = `${regMonthIn.value}-${String(d).padStart(2,'0')}`;
...
changeRegBtn.onclick = e => { e.preventDefault(); regTableWrapper.classList.add('hidden'); regSummarySection.classList.add('hidden'); loadRegBtn.classList.remove('hidden'); changeRegBtn.classList.add('hidden'); };

shareRegBtn2.onclick = e => { e.preventDefault(); const hdr = Register for ${regMonthIn.value}\nSchool: ${localStorage.getItem('schoolName')} | Class: ${localStorage.getItem('teacherClass')} | Section: ${localStorage.getItem('teacherSection')}; const lines = Array.from(regSummaryBody.querySelectorAll('tr')).map(r=>{ const td = r.querySelectorAll('td'); return ${td[0].textContent}: P:${td[1].textContent}, A:${td[2].textContent}, Lt:${td[3].textContent}, HD:${td[4].textContent}, L:${td[5].textContent}, %:${td[6].textContent}; }); window.open(https://wa.me/?text=${encodeURIComponent(hdr+'\n\n'+lines.join('\n'))}, '_blank'); };

downloadRegBtn2.onclick = e => { e.preventDefault(); const { jsPDF } = window.jspdf; const doc = new jsPDF('l','pt','a4'); doc.text(localStorage.getItem('schoolName'), 40, 30); doc.text(Class: ${localStorage.getItem('teacherClass')} | Section: ${localStorage.getItem('teacherSection')}, 40, 45); doc.text(Register for ${regMonthIn.value}, 40, 60); doc.autoTable({ html: '#registerTable', startY: 75, styles: { fontSize: 8 } }); doc.autoTable({ html: '#registerSummarySection table', startY: doc.lastAutoTable.finalY + 10, styles: { fontSize: 8 } }); doc.save('attendance_register.pdf'); };

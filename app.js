// app.js (part 1/3 - complete including unchanged sections)
...
(() => {
  ...
  window.addEventListener('DOMContentLoaded', async () => {
    ...
    applyFilter.onclick = () => {
      ...
      // If "All" checked → bulk export mode
      if (filterOptions.all) {
        ...
        roster.forEach(s => {
          // TODO: call per-student PDF functions here,
          // e.g. studentFullReportPDF(s, filterOptions);
        });
        hide(filterDialog);
      ...
      // Otherwise, re-render student table
      renderStudents();
      hide(filterDialog);
    };
    ...
    // renderStudents(), updateCounters(), resetViews() will be defined in part 2/3

  }); // end DOMContentLoaded
})();
// app.js (part 2/3)

// --- 6. COUNTERS & VIEW UTILITIES ---
...
// wire up class/section changes
$('teacherClassSelect').onchange   = () => { renderStudents(); updateCounters(); resetViews(); };
$('teacherSectionSelect').onchange = () => { renderStudents(); updateCounters(); resetViews(); };

// --- 7. RENDER STUDENTS (with search + filter) ---
  ...
  students
    ...
    .filter(s => {
      if (filterOptions.time) {
        // no student-specific date field, so skip
        return true;
      }
      return true;
    })
...
// Payment modal opener (full logic in part 3)
...
// Define generateFineReportPDF before binding
async function generateFineReportPDF() {
  ...
  if (type === 'date') {
    ...
    const m = amonth.value; const [y,mon]=m.split('-').map(Number);
    fromDate = `${m}-01`; toDate = `${m}-${String(new Date(y,mon,0).getDate()).padStart(2,'0')}`;
    ...
    alert('Select valid period'); return;
  }
  ...
  for (const [date,recs] of Object.entries(attendanceData)) {
    ...
    students.forEach(s => {
      ...
      const amt = fineRates[status] || 0;
      if (amt > 0) rows.push([s.adm, s.name, s.cls, s.sec, date, status, amt]);
    });
  }

...
window.addEventListener('DOMContentLoaded', ()=>{
  ...
  function renderStudents(){
    ...
    students.forEach((s,i)=>{
      ...
      tr.querySelector('.delete').onclick = ()=>{ if(confirm('Delete?')){ students.splice(i,1); saveStudents(); renderStudents(); }};
      tr.querySelector('.share').onclick = ()=>{
        const setup = `${localStorage.getItem('schoolName')} | Class: ${localStorage.getItem('teacherClass')} | Section: ${localStorage.getItem('teacherSection')}`;
        const msg = `School: ${setup}\nName: ${s.name}\nAdm#: ${s.adm}\nParent: ${s.parent}\nContact: ${s.contact}\nOccupation: ${s.occupation}\nAddress: ${s.address}`;
  ...
  addBtn.onclick = ()=>{
    const vals = inputs.map(i=>i.value.trim()); if(!vals[0]||!vals[1]) return alert('Name & Adm# required');
  ...
  $('shareAll').onclick = ()=>{
    const setup = `${localStorage.getItem('schoolName')} | Class: ${localStorage.getItem('teacherClass')} | Section: ${localStorage.getItem('teacherSection')}`;
    const text = students.map(s=>`School: ${setup}\nName: ${s.name}\nAdm#: ${s.adm}\nParent: ${s.parent}\nContact: ${s.contact}\nOccupation: ${s.occupation}\nAddress: ${s.address}`).join('\n---\n');
  ...
  loadAtt.onclick = ()=>{
    if(!dateIn.value) return alert('Pick date'); attList.innerHTML='';
    students.forEach((s,i)=>{
      ...
      if(saved){ const b=row.querySelector(`[data-code="${saved}"]`); b.style.background=colors[saved]; b.style.color='#fff'; }
      row.querySelectorAll('.att-btn').forEach(btn=>btn.onclick=()=>{
        ...
        const c=btn.dataset.code; btn.style.background=colors[c]; btn.style.color='#fff';
  ...
  saveAtt.onclick = ()=>{
    ...
    students.forEach(s=>{
      const st=attendanceData[d][s.roll]; const tr=document.createElement('tr');
      tr.innerHTML=`<td>${s.name}</td><td>${st}</td><td><button class="send">Send</button></td>`;
      tr.querySelector('.send').onclick=()=>{
        window.open(`https://wa.me/${s.contact}?text=${encodeURIComponent(`Attendance for ${s.name} on ${d}: ${st}`)}`,'_blank');
      };

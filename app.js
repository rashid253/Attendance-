window.addEventListener('DOMContentLoaded', async () => {
  ...
  function updateCounters() {
    const cl = $('teacherClassSelect').value,
      sec = $('teacherSectionSelect').value;
    $('sectionCount').dataset.target = students.filter(
      (s) => s.cls === cl && s.sec === sec
    ).length;
  ...
  saveAttendanceBtn.onclick = async () => {
    ...
    attendanceSummaryDiv
      .querySelectorAll('.share-individual')
      .forEach((ic) => {
        ic.onclick = () => {
          const adm = ic.dataset.adm,
            st = students.find((x) => x.adm === adm);
  ...
  function renderAnalytics(stats, from, to) {
    ...
    pieChart = new Chart(pieCtx, {
      ...
      data: {
        ...
        datasets: [
          {
            data: Object.keys(analyticsStatusNames).map(
              (code) => totalCounts[code]
            ),

...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // disable type until target chosen
  $('analyticsType').disabled = true;

  // when target changes
  $('analyticsTarget').addEventListener('change', () => {
    $('analyticsType').disabled = false;
    hide($('analyticsSectionSelect'), $('analyticsSearch'), $('instructions'), $('analyticsContainer'), $('graphs'), $('analyticsActions'));
  });

  // when type changes
  $('analyticsType').addEventListener('change', () => {
    hide(adate, amonth, sems, seme, ayear, $('instructions'), $('analyticsContainer'), $('graphs'), $('analyticsActions'));
  ...
  $('loadAnalytics').onclick = () => {
    // same logic as before to compute lastAnalyticsStats, lastAnalyticsRange
    let from, to;
    if (atype.value === 'date') {
      from = to = adate.value;
  ...
  async function downloadAnalyticsHandler() {
    ...
    doc.autoTable({ startY: 40, head: [['#','Adm#','Name','P','A','Lt','HD','L','Total','%','Outstanding','Status']], body });

...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // 2. STUDENT REGISTRATION WITH CLASS/SECTION FILTER
  ...
  function onCellBlur(e) {
    ...
    const stu = list[idx];
    if (ci === 2) {
      if (!/^\d+$/.test(val)) return alert('Adm# must be numeric');
      if (allStudents.some(s => s.adm===val && s.roll!==stu.roll)) return alert('Duplicate Adm# not allowed');
    }
    if (ci>=1 && ci<=6) {
      stu[keys[ci-1]] = val;
  ...
  addStudentBtn.onclick = async ev => {
    ...
    const student = { name, adm, parent, contact, occupation: occ, address: addr, roll: Date.now(), cls, sec };
  ...
  deleteSelBtn.onclick = async ev => {
    ...
    const rollsToDel = Array.from(document.querySelectorAll('.sel:checked'))
                          .map(cb=>filteredStudents()[+cb.dataset.index].roll);
    allStudents = allStudents.filter(s=>!rollsToDel.includes(s.roll));
    await saveAllStudents();
    renderStudents();
  ...
  // 3. ATTENDANCE MARKING for filtered students
  ...
  saveAtt.onclick = async ev => {
    ...
    Array.from(attList.querySelectorAll('.attendance-actions')).forEach((btns, i) => {
      ...
      attendanceData[d][filteredStudents()[i].roll] = sel ? sel.dataset.code : 'A';
    });
  ...
  downloadAnalyticsBtn.onclick = ev => {
    ...
    doc.autoTable({ head:[['Name','P','A','Lt','HD','L','Total','%']], body:
      Array.from(analyticsContainer.querySelectorAll('tbody tr')).map(r=>
    ...
    alert('PDF generation complete. Starting download...'); doc.save('attendance_analytics.pdf');
  };

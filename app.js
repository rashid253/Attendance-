window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('shareRegistration').onclick = () => {
    ...
    const lines = students.filter(s => s.cls===cl&&s.sec===sec).map(s => {
      const tf = Object.entries(attendanceData).reduce((a,[d,r]) => a + (r[s.adm]==='A'?fineRates.A:0)+(r[s.adm]==='Lt'?fineRates.Lt:0)+(r[s.adm]==='L'?fineRates.L:0)+(r[s.adm]==='HD'?fineRates.HD:0), 0);
      const tp = (paymentsData[s.adm]||[]).reduce((a,p) => a+p.amount, 0);
      const out = tf - tp, totalDays=Object.keys(attendanceData).length;
  ...
  loadAttendanceBtn.onclick = () => {
    ...
    students.filter(s=>s.cls===cl&&s.sec===sec).forEach((stu,i)=>{
      ...
      Object.keys(statusNames).forEach(code=>{
        ...
        btn.onclick = ()=>{ btnsDiv.querySelectorAll('.att-btn').forEach(b=>{b.classList.remove('selected');b.style.background='';b.style.color='';}); btn.classList.add('selected'); btn.style.background=statusColors[code]; btn.style.color='#fff'; };
  ...
  $('applyAnalyticsFilter').onclick = () => {
    analyticsFilterOptions = Array.from(filterCheckboxes)
      .filter(cb => cb.checked)
      .map(cb => cb.value);
    analyticsDownloadMode = filterForm.querySelector('input[name="downloadMode"]:checked').value;
  ...
  function renderAnalytics(stats, from, to) {
    ...
    pieChart = new Chart(pieCtx, {
      type: 'pie',
      data: { labels: ['Outstanding'], datasets: [{ data: [filtered.reduce((a, st) => a + st.outstanding, 0)] }] }
    });
  ...
  shReg.onclick = () => {
    ...
    const rows = Array.from(rb.children).map(tr =>
      Array.from(tr.children)
        .map(td => td.querySelector('.status-text') ? td.querySelector('.status-text').textContent : td.textContent)
        .join(' ')
    );
    window.open(`https://wa.me/?text=${encodeURIComponent(header + '\n' + rows.join('\n'))}`, '_blank');
  };

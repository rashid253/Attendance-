...
\nSection: ${localStorage.getItem('teacherSection')}`;
  ...
  loadAnalyticsBtn.onclick = ev => {
    ...
    if (analyticsType.value === 'date') {
      ...
      const [ys,ms] = semesterStartInput.value.split('-').map(Number);
      const [ye,me] = semesterEndInput.value.split('-').map(Number);
      from = `${semesterStartInput.value}-01`;
      to   = `${semesterEndInput.value}-${new Date(ye,me,0).getDate()}`;
  ...
  // -----------------------------
  // 5. ATTENDANCE REGISTER (fixed summary)
  // -----------------------------
  ...
  loadReg.onclick = e => {
    ...
    // summary: include roll for lookup

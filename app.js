...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('fineAbsent').value=fineRates.A; $('fineLate').value=fineRates.Lt; $('fineLeave').value=fineRates.L;
  $('fineHalfDay').value=fineRates.HD; $('eligibilityPct').value=eligibilityPct;
  ...
  // --- 4. COUNTERS & UTILS ---
  function animateCounters(){document.querySelectorAll('.number').forEach(span=>{const t=+span.dataset.target;let c=0,step=Math.max(1,t/100);(function u(){c+=step;span.textContent=c<t?Math.ceil(c):t; c<t&&requestAnimationFrame(u);})();});}
  ...
  function renderStudents(){
    ...
    students.forEach((s,i)=>{ if(s.cls!==cl||s.sec!==sec) return; idx++;
      ...
      const paid=(paymentsData[s.adm]||[]).reduce((a,p)=>a+p.amount,0),out=fine-paid,pct=tot?stats.P/tot*100:0,status=(out>0||pct<eligibilityPct)?'Debarred':'Eligible';
  ...
  saveAttendanceBtn.onclick = async () => {
    ...
    attendanceSummaryDiv.querySelectorAll('.share-individual').forEach(ic => ic.onclick = () => {
      const a = ic.dataset.adm, st = students.find(x => x.adm === a);
  ...
  downloadAttendanceBtn.onclick = async () => {
    ...
    const y = doc.lastAutoTable.finalY + 10;
    doc.setFontSize(12);
    doc.text(`Totals → P: ${P}, A: ${A}, Lt: ${Lt}, HD: ${HD}, L: ${L}`, 14, y);
    const blob = doc.output('blob');
  ...
  let barChart, pieChart, downloadAnalyticsHandler, shareAnalyticsHandler;

  // Show/hide period pickers based on type selection
  function updatePeriodPickers() {
    ...
    switch (atype.value) {
      case 'date':
        adate.closest('.form-group')?.classList.remove('hidden');
        break;
      case 'month':
        amonth.closest('.form-group')?.classList.remove('hidden');
        break;
      case 'semester':
        sems.closest('.form-group')?.classList.remove('hidden');
        seme.closest('.form-group')?.classList.remove('hidden');
        break;
      case 'year':
        ayear.closest('.form-group')?.classList.remove('hidden');
        break;
      case 'student':
        asearch.closest('.form-group')?.classList.remove('hidden');
        break;
      default:
        break;
    }
  ...
  function setupAnalyticsHandlers() {
    ...
    downloadAnalyticsHandler = async () => {
      ...
      const { from, to } = lastAnalyticsRange;
      doc.setFontSize(18);
      doc.text('Attendance Analytics and Student Report', 14, 16);
      doc.setFontSize(12);
      doc.text($('setupText').textContent, 14, 24);
      ...
      if (analyticsDownloadMode === 'combined') {
        ...
        doc.setFontSize(12);
        doc.text('HOD Signature', pageWidth - 60, pageHeight - 20);
      } else {
        for (let i = 0; i < lastAnalyticsStats.length; i++) {
          const st = lastAnalyticsStats[i];
          if (i > 0) doc.addPage();
          doc.setFontSize(16);
          doc.text(`Report for ${st.name} (Adm#: ${st.adm})`, 14, 16);
          doc.setFontSize(12);
          doc.text($('setupText').textContent, 14, 24);
          doc.text(`Period: ${from} to ${to}`, 14, 30);
          ...
          doc.autoTable({ startY: 40, head: [['Detail', 'Value']], body: tableBody, styles: { cellPadding: 2 } });
          doc.setFontSize(12);
          doc.text('HOD Signature', pageWidth - 60, pageHeight - 20);
        }
      }
  ...
  function renderAnalytics(stats, from, to) {
    ...
    downloadAnalyticsHandler && $('downloadAnalytics').removeEventListener('click', downloadAnalyticsHandler);
    shareAnalyticsHandler    && $('shareAnalytics').removeEventListener('click', shareAnalyticsHandler);

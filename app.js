...
// Features: Setup, Financial Settings with global Fine Mode, Student Registration w/ Admission Date, Attendance Marking, Analytics, Attendance Register, PDF Export, WhatsApp Sharing, PWA Service Worker

window.addEventListener('DOMContentLoaded', async () => {
  // 0. Service Worker registration (ignore missing sw.js)
  ...
  // 2. Load persisted or default values
  ...
  $('saveSetup').onclick = async () => {
    const d = {
      ...
      grade:  $('teacherClassSelect').value,
      section:$('teacherSectionSelect').value
    };
    if (!d.school||!d.grade||!d.section) return alert('Complete setup');
    await Promise.all([
      save('setup', d),
      save('schoolName', d.school),
      save('teacherClass', d.grade),
      save('teacherSection', d.section)
    ]);
    renderSetup(d);
  ...
  // 6. FINANCIAL SETTINGS & GLOBAL FINE MODE
  ...
  [ $('modeManual'), $('modeSmart') ].forEach(r=>{
    r.onchange = async ()=>{
      ...
      // update existing student fineMode
      students = students.map(s=>({ ...s, fineMode: calcMode }));
  ...
  function renderStudents() {
    ...
    students.forEach(s=>{
      ...
      idx++;
      // compute attendance after admission date
      ...
      const out = fine - paid;
      const total = stats.P+stats.A+stats.Lt+stats.L+stats.HD;
  ...
  $('analyticsType').onchange = () => {
    ...
    if(v==='date') show($('analyticsDate'));
    if(v==='month') show($('analyticsMonth'));
    if(v==='semester') { show($('semesterStart')); show($('semesterEnd')); }
    if(v==='year') show($('yearStart'));
  };
  ...
  $('loadAnalytics').onclick = () => {
    ...
    if(t==='date') { from=to=$('analyticsDate').value; }
      ...
      const ym=$('analyticsMonth').value; const [y,m]=ym.split('-');
      from=`${ym}-01`; to=`${ym}-${String(new Date(y,m,0).getDate()).padStart(2,'0')}`;
    }
    else if(t==='semester') {
      const s=$('semesterStart').value,e=$('semesterEnd').value;
      from=`${s}-01`; const [ey,em]=e.split('-'); to=`${e}-${String(new Date(ey,em,0).getDate()).padStart(2,'0')}`;
    }
    ...
    if($('analyticsTarget').value==='student') {
      ...
      if(!pool.length) return alert('No student found');
    }
    ...
    Object.entries(attendanceData).forEach(([d,recs])=>{
      stats.forEach(st=>{
        if(d<st.admissionDate||d<from||d>to) return;
        const c=recs[st.adm]||'A'; st[c]++; st.total++;
      });
    });
    stats.forEach(st=>{
      const tf=st.A*fineRates.absent+st.Lt*fineRates.late+st.L*fineRates.leave+st.HD*fineRates.halfDay;
      const paid=(paymentsData[st.adm]||[]).reduce((sum,p)=>sum+p.amount,0);
    ...
    stats.forEach((st,i)=>{
      ...
      tr.innerHTML=`
        <td>${i+1}</td><td>${st.adm}</td><td>${st.name}</td><td>${st.admissionDate}</td><td>${st.fineMode}</td>
    ...
    $('instructions').textContent=`Period: ${lastRange.from} to ${lastRange.to}`;
  ...
  $('downloadAnalytics').onclick = () => {
    const doc=new jspdf.jsPDF(); doc.setFontSize(16); doc.text('Analytics Report',14,16);
    doc.setFontSize(12); doc.text(`Period: ${lastRange.from} to ${lastRange.to}`,14,24);
  ...
  $('loadRegister').onclick = () => {
    ...
    roster.forEach((s,i)=>{
      ...
      for(let d=1;d<=days;d++){
        ...
        const style=code==='A'?'':`style="background:${{P:'green',A:'red',Lt:'orange',HD:'blue',L:'purple'}[code]};color:#fff"`;
    ...
    document.querySelectorAll('.reg-cell').forEach(cell=>{
      cell.onclick=()=>{
        const span=cell.querySelector('.status-text'), codes=['P','A','Lt','HD','L'];
        let i=codes.indexOf(span.textContent), c=codes[(i+1)%codes.length];
        span.textContent=c; if(c==='A'){ cell.style.background=''; cell.style.color=''; } else { cell.style.background='red'; cell.style.color='#fff'; }
      };
    });
  ...
  $('shareRegister').onclick = () => {
    const m=$('registerMonth').value; const header=`Register ${m}`;
    const lines=Array.from($('registerBody').children).map(tr=>{
      const name=tr.children[2].textContent, statuses=Array.from(tr.children).slice(3).map(td=>td.textContent).join('');
      return `*${name}*: ${statuses}`;
    }).join('\n');
    window.open(`https://wa.me/?text=${encodeURIComponent(header+'\n\n'+lines)}`,'_blank');
  };

  // hide initial action buttons
  resetViews();

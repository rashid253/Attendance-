...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('studentsBody').addEventListener('change',
    e => e.target.classList.contains('sel') && toggleButtons()
  );
});
...
saveAttendanceBtn.onclick = async () => {
  ...
  roster.forEach(s => {
    const code = attendanceData[date][s.adm], text = statusNames[code]||'Absent';
    tbl.innerHTML += `<tr>
      <td>${s.name}</td>
      <td>${text}</td>
...
loadA.onclick = () => {
  ...
  if (atype.value==='date') { from = to = adate.value; }
    ...
    from = `${sems.value}-01`;
    const [y2,m2] = seme.value.split('-').map(Number);
    to   = `${seme.value}-${new Date(y2,m2,0).getDate()}`;
  }
  ...
  stats.forEach((st,i) => {
    const pTxt = st.total ? ((st.P/st.total)*100).toFixed(1) : '0.0';
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td>${i+1}</td>
      <td>${st.adm}</td>
...
changeReg.onclick = () => {
  hide(changeReg, dlReg, shReg); show(saveReg);
};

...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('shareRegistration').onclick = () => {
    ...
    const lines = students.filter(s=>s.cls===cl&&s.sec===sec).map(s=>{
      ...
      return `${s.name} (Adm#${s.adm}): PKR ${out}, ${status}`;
    }).join('\n');
    ...
    roster.forEach((stu,i)=>{
      ...
      ['P','A','Lt','HD','L'].forEach(code=>{
        ...
        btn.onclick = () => {
          ...
          btn.style.background = {'P':'#4caf50','A':'#f44336','Lt':'#ffeb3b','HD':'#ff9800','L':'#2196f3'}[code];
          btn.style.color = '#fff';
        };
  ...
  $('saveAttendance').onclick = async () => {
    ...
    sum.innerHTML = `<h3>Attendance: ${date}</h3><table><tr><th>Name</th><th>Status</th></tr>` +
    ...
    show($('resetAttendance'), $('downloadAttendancePDF'), $('shareAttendanceSummary'), sum);
  };
  ...
  const atg          = $('analyticsTarget'),
        ...
        graphs       = $('graphs'),
        filterBtn    = $('analyticsFilterBtn'),
        filterPanel  = $('analyticsFilterPanel'),
        filterChecks = document.querySelectorAll('.analytics-filter'),
        clearFilters = $('clearFilters'),
        downloadBtn  = $('downloadReports'),
  ...
  downloadBtn.addEventListener('click', () => {
    ...
    if (mode==='filtered') {
      ...
      rows.forEach((row,i) => {
        ...
        doc.text(`Report for ${c[2]} (Adm#: ${c[1]})`,10,20);
        doc.autoTable({
          startY:30,
          head:[['P','A','Lt','HD','L','Total','%','Outstanding','Status']],
          body:[[c[3],c[4],c[5],c[6],c[7],c[8],c[9],c[10],c[11]]]
        });
  ...
  loadA.onclick = () => {
    ...
    stats.forEach((st,i)=>{
      ...
      tr.innerHTML = `<td>${i+1}</td><td>${st.adm}</td><td>${st.name}</td>
        ...
        <td>${st.total}</td><td>${pct}%</td><td>PKR ${st.outstanding}</td><td>${st.outstanding>0?'Debarred':'Eligible'}</td>`;
      tbody.appendChild(tr);
    });
    ...
    students.filter(s=>s.cls===$('teacherClassSelect').value&&s.sec===$('teacherSectionSelect').value)
      .forEach((s,i)=>{
        ...
        for(let d=1;d<=days;d++){
          ...
          const style = c==='A'?'':`style="background:${{'P':'#4caf50','A':'#f44336','Lt':'#ffeb3b','HD':'#ff9800','L':'#2196f3'}[c]};color:#fff"`;
  ...
  $('downloadRegister').onclick = () => {
    ...
    doc.autoTable({ startY:24, html:'#registerTable', styles:{fontSize:10} });
    doc.save('attendance_register.pdf');
  };

  $('shareRegister').onclick = () => {
    const header=`Attendance Register\n${$('setupText').textContent}`;
    const rows=Array.from($('registerBody').children).map(tr=>Array.from(tr.children)

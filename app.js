...
<body>
  <!-- (Your existing HTML elements: setup forms, tables, buttons, modals, etc.) -->
  ...
  window.addEventListener('DOMContentLoaded', async () => {
      ...
      const lines = students
        ...
        .map(s => {
          const totalFine = Object.entries(attendanceData).reduce(
            (sum,[d,r]) => sum + (r[s.adm]==='A'?fineRates.A:0)+(r[s.adm]==='Lt'?fineRates.Lt:0)+(r[s.adm]==='L'?fineRates.L:0)+(r[s.adm]==='HD'?fineRates.HD:0),
            0
          );
    ...
    downloadAnalyticsBtn.onclick = async () => {
      ...
      if (analyticsDownloadMode === 'combined') {
        ...
        // Individual reports
        for (const stat of lastAnalyticsStats) {
          const doc = new jspdf.jsPDF();
          doc.setFontSize(18);
          doc.text(`Analytics: ${stat.name}`, 14, 16);
          doc.setFontSize(12);
          ...
          const fileName = `analytics_${stat.adm}.pdf`;
          const blob = doc.output('blob');
          doc.save(fileName);
          await sharePdf(blob, fileName, `Analytics: ${stat.name}`);
        }
      }
    };
    ...
    loadA.onclick = () => {
      ...
      const stats = pool.map(s=>({
        adm: s.adm, name: s.name, P:0, A:0, Lt:0, HD:0, L:0, total:0
      }));

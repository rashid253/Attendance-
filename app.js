...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('saveSetup').onclick = async e => {
    ...
    if(!sc) return alert('Required');
    const cl = $('teacherClassSelect').value, sec= $('teacherSectionSelect').value;
    if(!cl||!sec) return alert('Pick both');
  ...
  $('editSettings').onclick = () => { show($('financialForm'), $('saveSettings')); hide($('settingsCard'), $('editSettings')); };

  // 6. Counters & Views
  function animateCounters() {
    document.querySelectorAll('.number').forEach(span => {
      const tgt = +span.dataset.target; let v=0, step=Math.max(1,tgt/100);
  ...
  function resetViews() {
    hide($('attendanceBody'),$('saveAttendance'),$('attendanceSummary'),
         $('downloadAttendancePDF'),$('shareAttendanceSummary'),
         $('registerTableWrapper'),$('downloadRegister'),$('shareRegister'));
  }
  ...
  function renderStudents() {
    ...
    students.forEach(s=>{
      ...
      Object.entries(attendanceData).forEach(([_,r])=> stats[r[s.adm]||'A']++);
  ...
  document.body.addEventListener('click',e=>{
    if(e.target.classList.contains('add-payment-btn')) {
      const adm=e.target.dataset.adm;
      const amt=prompt(`Payment for ${adm}:`,'0');
      if(amt===null) return;
  ...
  const statusCols={P:'#4caf50',A:'#f44336',Lt:'#ff9800',HD:'#ffb300',L:'#2196f3'};
  $('loadAttendance').onclick=()=>{
    const roster=students.filter(s=>s.cls===$('teacherClassSelect').value&&s.sec===$('teacherSectionSelect').value);
    const container=$('attendanceBody'); container.innerHTML='';
    roster.forEach(stu=>{
      const row=document.createElement('div'); row.className='attendance-row';
      const nd=document.createElement('div'); nd.className='attendance-name'; nd.textContent=stu.name;
      const bd=document.createElement('div'); bd.className='attendance-buttons';
      Object.keys(statusNames).forEach(code=>{
        const btn=document.createElement('button'); btn.textContent=code;
        btn.onclick=()=>{ bd.querySelectorAll('button').forEach(b=>b.style=''); btn.style.background=statusCols[code]; };
        bd.appendChild(btn);
      });
      row.append(nd,bd); container.appendChild(row);
    });
    show($('saveAttendance'));
  };
  $('saveAttendance').onclick=async()=>{
    ...
    document.querySelectorAll('.attendance-row').forEach((row,i)=>{
      const code=row.querySelector('button[style]')?.textContent||'A';
    ...
    alert('Saved'); show($('downloadAttendancePDF'),$('shareAttendanceSummary'));
  };

  // PDF preview & share modal
  async function previewAndSharePDF(doc,filename='report.pdf'){
    const blob=doc.output('blob'), url=URL.createObjectURL(blob), modal=$('pdfPreviewModal');
    $('pdfPreviewFrame').src=url; show(modal);
    $('closePreview').onclick=()=>{ hide(modal); URL.revokeObjectURL(url); };
    $('downloadFromPreview').onclick=()=>{ const a=document.createElement('a'); a.href=url; a.download=filename; a.click(); };
    ...
    $('shareWhatsapp').onclick=()=>{ window.open(`https://wa.me/?text=${encodeURIComponent('Download PDF: '+url)}`,'_blank'); };
    $('shareFacebook').onclick=()=>{ window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`,'_blank'); };
    $('shareEmail').onclick=()=>{ location.href=`mailto:?subject=${encodeURIComponent('PDF Report')}&body=${encodeURIComponent('Download PDF: '+url)}`; };
  }

  // Download handlers
  $('downloadAttendancePDF').onclick=()=>{
    const doc=new jspdf.jsPDF(); doc.text('Attendance Report',14,16); doc.autoTable({ html:'#attendanceBody' });
    previewAndSharePDF(doc,`attendance_${$('dateInput').value}.pdf`);
  };
  $('downloadAnalytics').onclick=()=>{
    const doc=new jspdf.jsPDF(); doc.text('Analytics Report',14,16); doc.autoTable({ html:'#analyticsTable' });
    previewAndSharePDF(doc,'analytics_report.pdf');
  };
  $('downloadRegister').onclick=()=>{
    const doc=new jspdf.jsPDF({orientation:'landscape'}); doc.text('Register',14,16); doc.autoTable({ html:'#registerTable' });
  ...
  $('loadRegister').onclick=()=>{
    ...
    if(!dates.length) return alert('No records');
    ...
    students.filter(s=>s.cls===$('teacherClassSelect').value&&s.sec===$('teacherSectionSelect').value)
      .forEach((s,i)=>{
        ...
        dates.forEach(date=>{ row+=`<td>${attendanceData[date]?.[s.adm]||'A'}</td>`; });

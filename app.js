...
Address: ${s.address}`;
  ...
  addBtn.onclick = ev => {
    ...
    // admissionNo numeric
    if (!/^\d+$/.test(vals[1])) return alert('Admission # must be numeric');
    // contact numeric and 7–15 digits
  ...
  const shareSum=$('shareAttendanceSummary'), downloadAttPDF=$('downloadAttendancePDF');
  loadAtt.onclick=e=>{
    e.preventDefault();
    ...
    students.forEach(s=>{
      ...
      ['P','A','Lt','HD','L'].forEach(code=>{
        ...
        b.onclick=ev2=>{ ev2.preventDefault(); Array.from(btns.children).forEach(x=>{x.style.background='';x.style.color='var(--dark)'}); b.style.background=colors[code]; b.style.color='#fff'; };
        btns.append(b);
      });
  ...
  resetAtt.onclick=e=>{
    e.preventDefault();
    resSec.classList.add('hidden'); $('attendance-section').classList.remove('hidden');
    attList.innerHTML=''; saveAtt.classList.add('hidden'); summaryBody.innerHTML='';
  };
  shareSum.onclick=e=>{
    e.preventDefault();
    const rows=[...summaryBody.querySelectorAll('tr')].map(r=>r.textContent.trim()).join('\n');
    window.open(`https://wa.me/?text=${encodeURIComponent(rows)}`,'_blank');
  };
  downloadAttPDF.onclick=e=>{
    e.preventDefault();
    const { jsPDF } = window.jspdf;
    const doc=new jsPDF('p','pt','a4');
    doc.autoTable({
      head:[[ 'Name','Status' ]],
      body:[...summaryBody.querySelectorAll('tr')].slice(1).map(r=>{
  ...
  analyticsType.onchange=()=>{
    [analyticsDate,analyticsMonth,semesterStart,semesterEnd,yearStart,
     instructions,analyticsContainer,graphs,analyticsActions,resetAnalyticsBtn]
  ...
  loadAnalyticsBtn.onclick=e=>{
    ...
    instructions.textContent=`Analytics from ${from} to ${to}`;
    instructions.classList.remove('hidden');
    resetAnalyticsBtn.classList.remove('hidden');
    // charts
    const labels=stats.map(s=>s.name),
          dataPct=stats.map(s=>s.total?(s.P/s.total*100).toFixed(1):0);
    if(barChart) barChart.destroy();
    barChart=new Chart(barCtx,{type:'bar',data:{labels,datasets:[{label:'% Present',data:dataPct}]},options:{responsive:true,maintainAspectRatio:true}});
    const agg=stats.reduce((a,s)=>{['P','A','Lt','HD','L'].forEach(c=>a[c]+=s[c]);return a;},{P:0,A:0,Lt:0,HD:0,L:0});
    if(pieChart) pieChart.destroy();
    pieChart=new Chart(pieCtx,{type:'pie',data:{labels:['P','A','Lt','HD','L'],datasets:[{data:Object.values(agg)}]},options:{responsive:true,maintainAspectRatio:true}});
  ...
  downloadAnalytics.onclick=e=>{
    ...
    doc.text(instructions.textContent,40,40);
    ...
    doc.addPage();
    doc.text('Bar Chart',40,40);
    doc.addImage(barImg,'PNG',40,60,500,200);
    doc.addPage();
    doc.text('Pie Chart',40,40);
    doc.addImage(pieImg,'PNG',40,60,500,200);

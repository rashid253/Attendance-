...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    filteredStudents().forEach((st,idx)=>{
      ...
      if(btnSaveReg.classList.contains('hidden')){
        tr.querySelector('.share-one').onclick=()=>{
          const hdr=`*Attendance* ${schoolInput.value} ${classSelect.value}-${sectionSelect.value}`;
  ...
  btnAddStudent.onclick=async e=>{e.preventDefault();
    ...
    if(!/^\d{7,15}$/.test(cont)) return alert('Contact 7–15 digits');
  ...
  // --- INLINE EDIT/DELETE/REGISTRATION ---
  btnEditSel.onclick=e=>{ /* inline-edit code here */ };
  btnDeleteSel.onclick=e=>{ /* delete code here */ };
  btnSaveReg.onclick=e=>{ /* save registration code here */ };
  btnEditReg.onclick=e=>{ /* edit registration code here */ };
  btnShareReg.onclick=e=>{ /* share registration code here */ };
  btnDownloadReg.onclick=e=>{ /* download registration PDF code here */ };

  // --- ATTENDANCE MARKING & SUMMARY ---
  btnLoadAtt.onclick=e=>{ /* load attendance code here */ };
  btnSaveAtt.onclick=e=>{ /* save attendance code here */ };
  btnResetAtt.onclick=()=>{ /* reset attendance code here */ };
  btnShareAtt.onclick=()=>{ /* share attendance code here */ };
  btnDownloadAtt.onclick=()=>{ /* download attendance PDF code here */ };

  // --- ANALYTICS ---
  function resetAnalytics(){
    $('labelSection').classList.add('hidden'); analyticsSectionSelect.classList.add('hidden');
    $('labelFilter').classList.add('hidden'); analyticsFilter.classList.add('hidden');
  ...
  resetAnalytics();
  selectAnalyticsTarget.value='class'; selectAnalyticsTarget.dispatchEvent(new Event('change'));

  selectAnalyticsTarget.onchange=()=>{
    resetAnalytics();
    if(selectAnalyticsTarget.value==='class'){ selectAnalyticsType.disabled=false; }
    if(selectAnalyticsTarget.value==='section'){
      $('labelSection').classList.remove('hidden'); analyticsSectionSelect.classList.remove('hidden');
      selectAnalyticsType.disabled=false;
    }
    if(selectAnalyticsTarget.value==='student'){
      $('labelFilter').classList.remove('hidden'); analyticsFilter.classList.remove('hidden');
    }
  };
  ...
  analyticsFilter.onchange=()=>{
    ...
    choice.setChoices(filteredStudents().map(s=>({value:s.roll,label:`${s.name} — ${s.parent} — ${s.adm}`})),'value','label',true);
  ...
  selectAnalyticsType.onchange=()=>{
    ...
    if(selectAnalyticsType.value==='semester'){
      inputSemesterStart.classList.remove('hidden'); inputSemesterEnd.classList.remove('hidden');
    }
  ...
  btnLoadAnalytics.onclick=e=>{
    ...
    if(selectAnalyticsTarget.value==='student'){
      const roll=analyticsStudentInput.value; pool=students.filter(s=>String(s.roll)===roll);
    }

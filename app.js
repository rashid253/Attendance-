...
window.addEventListener('DOMContentLoaded', ()=> {
    ...
    students.forEach((s,i) => {
      ...
      tr.querySelector('.edit').onclick = () => {
        ['name','adm','parent','contact','occupation','address'].forEach(key => {
          s[key] = prompt(`Edit ${key}`, s[key]) || s[key];
        });
        saveStudents();
        renderStudents();
      };
  ...
  deleteSelected.onclick = () => {
    ...
    const toDel = [...document.querySelectorAll('.selectStudent:checked')]
  ...
  $('shareAttendanceSummary').onclick = () => {
    const lines = [...summaryBody.querySelectorAll('tr')].map(r=>{
      ...
      const stat = r.children[1].textContent;
      return `Name: ${name}\nStatus: ${stat}`;
    }).join('\n---\n');
  ...
  $('downloadAttendanceSummary').onclick = () => {
    ...
    let y = 20;
    doc.setFontSize(12);
    doc.text(`Attendance Summary - ${dateIn.value}`, 20, y);
    y += 20;
    doc.autoTable({
      html: summaryBody.closest('table'),
      startY: y,
  ...
  function buildDates(){
    ...
    function push(s,e){
      let c = new Date(s);
      while(c<=e){ arr.push(c.toISOString().slice(0,10)); c.setDate(c.getDate()+1); }
    }
    if(v==='date'){
    ...
    if(v==='semester'){
      let [ys,ms]=semStart.value.split('-'), [ye,me]=semEnd.value.split('-');
      push(new Date(ys,ms-1,1), new Date(ye,me,0));
    }
    if(v==='year'){
      let y=+yrStart.value;
      push(new Date(y,0,1), new Date(y,11,31));
    }
    return arr;
  }
  ...
  loadAnalytics.onclick = ()=>{
    ...
    const summary = students.map(s=>{
      ...
      dates.forEach(d=>{ const st=(dataA[d]||{})[s.roll]||''; if(st) cnt[st]++; });
  ...
  shareAnalytics.onclick = ()=>{
    ...
    const txt = summaryData.map(r=>
      `Name: ${r.name}\nPresent: ${r.P}\nLate: ${r.Lt}\nHalfDay: ${r.HD}\nAbsent: ${r.A}\nLeave: ${r.L}\nTotal: ${r.total}\n%: ${r.pct}%\nElig: ${r.pct>=THRESHOLD?'Yes':'No'}`
    ).join('\n---\n');
  ...
  downloadAnalytics.onclick = ()=>{
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF('p','pt','a4'); let y=20;
    doc.setFontSize(12);

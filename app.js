...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('saveSetup').onclick = async e => {
    e.preventDefault();
    const sc = prompt('Enter school name:', 'My School');
    if (!sc) return alert('School name required');
  ...
  function resetViews() {
    hide(
      $('attendanceBody'), $('saveAttendance'), $('attendanceSummary'),
      $('downloadAttendancePDF'), $('shareAttendanceSummary'),
      $('registerTableWrapper'), $('saveRegister'), $('downloadRegister'),
      $('shareRegister')
    );
  }
  ...
  $('addStudent').onclick = async e => {
    ...
    const c=$('parentContact').value.trim(), o=$('parentOccupation').value.trim();
    const a=$('parentAddress').value.trim(), cl=$('teacherClassSelect').value;
    const sec=$('teacherSectionSelect').value;
  ...
  document.body.addEventListener('click', e => {
    if (e.target.classList.contains('add-payment-btn')) {
      ...
      const amt = prompt(`Enter payment for ${adm}:`, '0');
      if (amt===null) return;
  ...
  $('loadAttendance').onclick = () => {
    ...
    roster.forEach(stu => {
      ...
      Object.keys(statusNames).forEach(code => {
        ...
        btn.onclick = () => {
          btns.querySelectorAll('button').forEach(b=>{ b.style=''; });
          btn.style.background = statusCols[code];
        };
  ...
  $('saveAttendance').onclick = async () => {
    ...
    show($('downloadAttendancePDF'), $('shareAttendanceSummary'));
  };

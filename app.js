...
window.addEventListener('DOMContentLoaded', async () => {
  // --- 0. Request persistent storage so IndexedDB isn’t cleared on data purge ---
  ...
  // --- 4. IndexedDB helpers (idb-keyval) ---
  if (!window.idbKeyval) {
  ...
  // --- 11. STUDENT REGISTRATION & FINE/STATUS ---
  ...
  $('selectAllStudents').onclick = () => {
    document.querySelectorAll('.sel').forEach(c => {
      c.checked = $('selectAllStudents').checked;
    });
    toggleButtons();
  };
  ...
  $('shareRegistration').onclick = () => {
    ...
    const lines = students.filter(s => s.cls===cl && s.sec===sec).map(s => {
      const tf = Object.entries(attendanceData).reduce((a,[d,r]) =>
        a + (r[s.adm]==='A'?fineRates.A:0)
  ...
  loadA.onclick = () => {
    ...
    // 3) Build the pool of students
    const cls = $('teacherClassSelect').value;
    let pool = students.filter(s => s.cls === cls && s.sec === $('teacherSectionSelect').value);
    ...
    // 5) Render the analytics table & charts
    ...
    if (!analyticsFilterOptions.includes('all')) {
      filtered = stats.filter(st => analyticsFilterOptions.some(opt => {
        switch (opt) {
          ...
          case 'fine':       return (st.A + st.Lt + st.L + st.HD) > 0;
  ...
  loadReg.onclick = () => {
    ...
    rb.querySelectorAll('.reg-cell').forEach(cell => {
      cell.onclick = () => {
        ...
        const idx = (regCodes.indexOf(span.textContent) + 1) % regCodes.length;
  ...
  dlReg.onclick = async () => {
    // Download & share is already wired in downloadRegister handler above
    // We just trigger it:
    $('downloadRegister').click();
  };

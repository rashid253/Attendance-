...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  btnAddStudent.onclick=async e=>{e.preventDefault();
    const name=nameInput.value.trim(), parent=parentInput.value.trim(), cont=contactInput.value.trim(),
          occ=occInput.value.trim(), addr=addrInput.value.trim();
  ...
  btnSaveAtt.onclick=e=>{ /* ... */ };
  btnResetAtt.onclick=()=>{ /* ... */ };
  btnShareAtt.onclick=()=>{ /* ... */ };
  btnDownloadAtt.onclick=()=>{ /* ... */ };

  // ---- ANALYTICS ----
  const choicesStudent=new Choices(analyticsStudentInput,{searchEnabled:true,shouldSort:false,placeholderValue:'--Select student--',itemSelectText:''});
  ...
  selectAnalyticsTarget.onchange=()=>{
    ...
    if(t==='class'||t==='section') selectAnalyticsType.disabled=false;
    else analyticsFilter.classList.remove('hidden');
  };
  ...
  btnLoadAnalytics.onclick=e=>{
    ...
    divGraphs.classList.remove('hidden'); btnShareAnalytics.classList.remove('hidden'); btnDownloadAnalytics.classList.remove('hidden');
  };
  ...
  btnLoadReg.onclick=e=>{ e.preventDefault();
    ...
    filteredStudents().forEach((s,i)=>{/* build rows */});

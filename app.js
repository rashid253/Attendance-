...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const THRESHOLD = 75;
  const colorMap = { P: '#4CAF50', A: '#f44336', Lt: '#FFEB3B', HD: '#FF9800', L: '#2196F3' };
  ...
  addBtn.onclick = () => {
    ...
    [sName,admIn,pNameIn,pConIn,pOccIn,pAddrIn].forEach(inp=>inp.value='');
    renderStudents();
  };
  ...
  loadA.onclick = () => {
    ...
    students.forEach(s => {
      ...
      div.innerHTML = `<span>${s.name}</span><div class="attendance-actions">
        ${['P','A','Lt','HD','L'].map(code=>`<button class="att-btn" data-code="${code}">${code}</button>`).join('')}
      </div>`;
      div.querySelectorAll('.att-btn').forEach(btn => {
        btn.onclick = () => {
          ...
          btn.style.background = colorMap[code];
          btn.style.color = (code==='Lt' || code==='HD')? '#333':'#fff';
        };
      });
  ...
  saveA.onclick = () => {
    ...
    attList.querySelectorAll('.attendance-item').forEach((div,i) => {
      const btn = div.querySelector('.att-btn[style*="background"]');
      attendanceData[d][students[i].roll] = btn? btn.textContent : 'Not marked';
    });
  ...
  loadAnalytics.onclick = () => {
    ...
    tbl.innerHTML=`<tr><th>Name</th><th>P</th><th>Lt</th><th>HD</th><th>L</th><th>A</th><th>%</th><th>Elig</th></tr>` +
      summary.map(r=>`<tr><td>${r.name}</td><td>${r.P}</td><td>${r.Lt}</td><td>${r.HD}</td><td>${r.L}</td><td>${r.A}</td><td>${r.pct}%</td><td>${r.pct>=THRESHOLD?'✓':'✗'}</td></tr>`).join('');

window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('shareRegistration').onclick = () => {
    ...
    const lines = students
      ...
      .map(s => {
        // compute stats on only days teacher marked
        const stats = { P:0, A:0, Lt:0, HD:0, L:0 };
        for (let [d, rec] of Object.entries(attendanceData)) {
  ...
  function renderStudents() {
    ...
    students.forEach((s,i)=>{
      ...
      const stats={P:0,A:0,Lt:0,HD:0,L:0};
      for(let [d,rec] of Object.entries(attendanceData)) if(rec[s.adm]) stats[rec[s.adm]]++;
      const totalMarked = stats.P+stats.A+stats.Lt+stats.HD+stats.L;
      const totalFine = stats.A*fineRates.A + stats.Lt*fineRates.Lt + stats.L*fineRates.L + stats.HD*fineRates.HD;
      const paid = (paymentsData[s.adm]||[]).reduce((a,p)=>a+p.amount,0);
      const outstanding = totalFine - paid;

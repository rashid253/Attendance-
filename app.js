...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('editSelected').onclick = () => {
    document.querySelectorAll('.sel:checked').forEach(cb=>{
      ...
      tr.innerHTML = `
        ...
        <td><input value="${s.address}"></td><td></td>`;
    });
  ...
  resetAnalyticsBtn.onclick = e => {
    e.preventDefault(); analyticsType.value='';
    hide(analyticsDate, analyticsMonth, semesterStart, semesterEnd, yearStart,
  ...
  shareRegister.onclick = () => {
    const hdr=`Attendance Register: ${monthInput.value}`,
          rows=Array.from(registerBody.querySelectorAll('tr')).map(tr=>Array.from(tr.querySelectorAll('td')).map(td=>td.querySelector('.status-text')?td.querySelector('.status-text').textContent:td.textContent).join(' '));

...
window.addEventListener('DOMContentLoaded', () => {
  ...
  shareAtt.onclick = ev=>{
    ...
    const d=dateInput.value,
          hdr=`Date: ${d}\nSchool: ${localStorage.getItem('schoolName')}\nClass: ${localStorage.getItem('teacherClass')}\nSection: ${localStorage.getItem('teacherSection')}`,
          map={P:'Present',A:'Absent',Lt:'Late',HD:'Half Day',L:'Leave'},
          lines=students.map(s=>`${s.name}: ${map[attendanceData[d][s.roll]||'A']}`),
          total=students.length, pres=students.reduce((sum,s)=>sum+(attendanceData[d][s.roll]==='P'?1:0),0),
          pct=total?((pres/total)*100).toFixed(1):'0.0', remark=pct==100?'Best':pct>=75?'Good':pct>=50?'Fair':'Poor',
          summary=`Overall Attendance: ${pct}% | ${remark}`,
          msg=[hdr,'',...lines,'',summary].join('\n');
    window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`,'_blank');
  };
  ...
  // 4. ANALYTICS (fixed semester across years)
  const analyticsTarget      = $('analyticsTarget'),
        admInput             = $('studentAdmInput'),
  ...
  loadAnalyticsBtn.onclick = ev => {
    ...
    if(analyticsTarget.value==='class'){
      ...
      const adm=admInput.value.trim(); if(!adm) return alert('Enter Admission #');
      const stud=students.find(s=>s.adm===adm); if(!stud) return alert(`No student with Adm#: ${adm}`);
      stats=[{name:stud.name,roll:stud.roll,P:0,A:0,Lt:0,HD:0,L:0,total:0}];
    }
    Object.entries(attendanceData).forEach(([d,recs])=>{ const cur=new Date(d);
  ...
  const regMonthIn      = $('registerMonth'),
        ...
        regTableWrapper = $('registerTableWrapper'),

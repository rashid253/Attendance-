...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // 6. COUNTERS & UTILS ...
  function animateCounters(){ document.querySelectorAll('.number').forEach(span=>{
  ...
  function renderStudents(){
    ...
    students.forEach((s,i)=>{
      ...
      const paid=(paymentsData[s.adm]||[]).reduce((a,p)=>a+p.amount,0); const out=totalFine-paid;
      const totalDays=stats.P+stats.A+stats.Lt+stats.HD+stats.L;
      const pct=totalDays?stats.P/totalDays*100:0; const status=(out>0||pct<eligibilityPct)?'Debarred':'Eligible';
  ...
  function toggleButtons(){ const any=!!document.querySelector('.sel:checked');
    $('editSelected').disabled=!any; $('deleteSelected').disabled=!any;
  }
  document.getElementById('studentsBody').addEventListener('change',e=>e.target.classList.contains('sel')&&toggleButtons());
  ...
  $('addStudent').onclick=async e=>{
    ...
    const n=$('studentName').value.trim(), p=$('parentName').value.trim(), c=$('parentContact').value.trim(),
          o=$('parentOccupation').value.trim(), a=$('parentAddress').value.trim(), cl=$('teacherClassSelect').value, sec=$('teacherSectionSelect').value;
  ...
  $('downloadRegistrationPDF').onclick=()=>{ const doc=new jspdf.jsPDF(); doc.setFontSize(18); doc.text('Student List',14,16); doc.setFontSize(12); doc.text($('setupText').textContent,14,24); doc.autoTable({startY:32,html:'#studentsTable'}); const url=doc.output('bloburl'); window.open(url,'_blank'); doc.save('registration.pdf'); };
  ...
  $('loadAttendance').onclick=()=>{
    const dateInput=$('dateInput'); if(!dateInput.value){ alert('Please pick a date'); return; }
    ...
    roster.forEach((stu,i)=>{
      ...
      ['P','A','Lt','HD','L'].forEach(code=>{
        ...
        btn.onclick=()=>{
          ...
          btn.classList.add('selected'); btn.style.background={'P':'var(--success)','A':'var(--danger)','Lt':'var(--warning)','HD':'#FF9800','L':'var(--info)'}[code]; btn.style.color='#fff';
        };
  ...
  $('saveAttendance').onclick=async()=>{
    ...
    summary.querySelectorAll('.share-individual').forEach(ic=>ic.onclick=()=>{
      const adm=ic.dataset.adm, student=students.find(x=>x.adm===adm), code=attendanceData[date][adm];
      window.open(`https://wa.me/${student.contact}?text=${encodeURIComponent(`Dear Parent, your child was ${{'P':'Present','A':'Absent','Lt':'Late','HD':'Half-Day','L':'Leave'}[code]} on ${date}.`)}`,'_blank');
    });
  ...
  $('downloadAttendancePDF').onclick=()=>{ const doc=new jspdf.jsPDF(); doc.setFontSize(18); doc.text('Attendance Report',14,16); doc.setFontSize(12); doc.text($('setupText').textContent,14,24); doc.autoTable({startY:32,html:'#attendanceSummary table'}); const url=doc.output('bloburl'); window.open(url,'_blank'); doc.save(`attendance_${$('dateInput').value}.pdf`); };
  $('shareAttendanceSummary').onclick=()=>{ const cl=$('teacherClassSelect').value, sec=$('teacherSectionSelect').value, date=$('dateInput').value;
  ...
  // 10. ANALYTICS (existing + enhancements) …
  const atg=$('analyticsTarget'), asel=$('analyticsSectionSelect'), atype=$('analyticsType'),
        ...
        aacts=$('analyticsActions'), barCtx=$('barChart').getContext('2d'), pieCtx=$('pieChart').getContext('2d');
  let barChart, pieChart, lastAnalyticsShare='';

  atg.onchange=()=>{ atype.disabled=false; [asel,asearch].forEach(x=>x.classList.add('hidden')); [instr,acont,graphs,aacts].forEach(x=>x.classList.add('hidden'));
  ...
  atype.onchange=()=>{
    ...
    [instr,acont,graphs,aacts].forEach(x=>x.classList.add('hidden')); resetA.classList.remove('hidden');
  ...
  loadA.onclick=()=>{
    ...
    stats.forEach(st=>{ const tf=st.A*fineRates.A+st.Lt*fineRates.Lt+st.L*fineRates.L+st.HD*fineRates.HD;
      const tp=(paymentsData[st.adm]||[]).reduce((sum,p)=>sum+p.amount,0); st.outstanding=tf-tp;
    ...
    instr.textContent=`Period: ${from} to ${to}`; show(instr,acont,graphs,aacts); resetA.classList.remove('hidden');
  ...
  $('downloadAnalytics').onclick=()=>{ const doc=new jspdf.jsPDF(); doc.setFontSize(18); doc.text('Analytics Report',14,16); doc.setFontSize(12); doc.text($('setupText').textContent,14,24); doc.autoTable({startY:32,html:'#analyticsTable'}); const url=doc.output('bloburl'); window.open(url,'_blank'); doc.save('analytics_report.pdf'); };
  ...
  $('loadRegister').onclick=()=>{
    ...
    $('registerBody').innerHTML=''; students.filter(s=>s.cls===$('teacherClassSelect').value&&s.sec===$('teacherSectionSelect').value).forEach((s,i)=>{
      let row=`<td>${i+1}</td><td>${s.adm}</td><td>${s.name}</td>`;
      for(let d=1;d<=days;d++){ const key=`${m}-${String(d).padStart(2,'0')}`, c=(attendanceData[key]||{})[s.adm]||'A';
    ...
    document.querySelectorAll('.reg-cell').forEach(cell=>cell.onclick=()=>{
      const span=cell.querySelector('.status-text'), idx=['A','P','Lt','HD','L'].indexOf(span.textContent);
      const next=['A','P','Lt','HD','L'][(idx+1)%5]; span.textContent=next;
      if(next==='A'){ cell.style.background=''; cell.style.color=''; } else{ cell.style.background={'P':'var(--success)','A':'var(--danger)','Lt':'var(--warning)','HD':'#FF9800','L':'var(--info)'}[next]; cell.style.color='#fff'; }
    });
  ...
  $('saveRegister').onclick=async()=>{
    ...
    Array.from($('registerBody').children).forEach(tr=>{
      ...
      for(let d=1;d<=days;d++){ const code=tr.children[3+d-1].querySelector('.status-text').textContent;
        const key=`${m}-${String(d).padStart(2,'0')}`; attendanceData[key]=attendanceData[key]||{}; attendanceData[key][adm]=code;
      }
    });
  ...
  $('downloadRegister').onclick=()=>{
    ...
    doc.setFontSize(18); doc.text('Attendance Register',14,16); doc.setFontSize(12); doc.text($('setupText').textContent,14,24);

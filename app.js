...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  function animateCounters(){
    document.querySelectorAll('.number').forEach(span => {
      ...
      (function upd(){ c+=step; span.textContent=c<tgt?Math.ceil(c):tgt; if(c<tgt) requestAnimationFrame(upd); })();
    });
  }
  ...
  $('saveAttendance').onclick=async()=>{
    ...
    const sum=$('attendanceSummary'); sum.innerHTML=`<h3>Report ${date}</h3>`;
    ...
    sum.querySelectorAll('.share-individual').forEach(ic=>ic.onclick=()=>{
      const adm=ic.dataset.adm, code=attendanceData[date][adm];
      window.open(`https://wa.me/${students.find(x=>x.adm===adm).contact}?text=${encodeURIComponent(`Your child was ${code} on ${date}`)}`,'_blank');
    });
  ...
  // --- 13. Analytics & PDF with embedded charts ---
  ...
  loadA.onclick=()=>{
    ...
    stats.forEach((st,i)=>{ const pct=st.total?((st.P/st.total)*100).toFixed(1):'0.0'; const tr=document.createElement('tr'); tr.innerHTML=`<td>${i+1}</td><td>${st.adm}</td><td>${st.name}</td><td>${st.P}</td><td>${st.A}</td><td>${st.Lt}</td><td>${st.HD}</td><td>${st.L}</td><td>${st.total}</td><td>${pct}%</td><td>₹ ${st.outstanding}</td><td>${st.status}</td>`; tbody.appendChild(tr); });
    instr.textContent=`Period: ${from} to ${to}`; show(instr,acont,graphs,aacts);

    barChart?.destroy();
    barChart=new Chart(barCtx,{ type:'bar', data:{ labels:stats.map(st=>st.name), datasets:[{ label:'% Present', data:stats.map(st=>st.total?st.P/st.total*100:0) }] }, options:{ scales:{ y:{ beginAtZero:true, max:100 } } } });
  ...
  $('downloadAnalytics').onclick=()=>{
    ...
    doc.addImage(barImg,'PNG',14,24,180,80);
    doc.addPage();

...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderStudents(){
    ...
    students.forEach((s,i)=>{
      ...
      const out = totalFine - paid; const totalDays = stats.P+stats.A+stats.Lt+stats.HD+stats.L;
  ...
  // (editSelected, doneEditing, deleteSelected, saveRegistration, shareRegistration, downloadRegistrationPDF - same as original)
  ...
  loadA.onclick = ()=>{
    ...
    // render analytics table and charts...
    show(instr, acont, graphs, aacts);
  };

  $('shareAnalytics').onclick = ()=>{ const text=`Analytics (${analyticsFrom} to ${analyticsTo})\n`+stats.map((st,i)=>`${i+1}. ${st.adm} ${st.name}: ${((st.P/st.total)*100).toFixed(1)}% / PKR ${st.outstanding}`).join('\n'); window.open(`https://wa.me/?text=${encodeURIComponent(text)}`,'_blank'); };

  $('downloadAnalytics').onclick = ()=>{
    const school=$('setupText').textContent, margin=40;
    const doc=new jspdf.jsPDF({ orientation:'landscape', unit:'pt', format:'a4' });
    let y=60;
    doc.setFontSize(18).text(`${school} – Attendance Summary Report`,margin,y); y+=30;
    doc.setFontSize(12).text(`Period: ${analyticsFrom} – ${analyticsTo}`,margin,y); y+=30;
    doc.autoTable({ startY:y, head:[['Adm#','Name','% Present','Outstanding','Status']],
      body:stats.map(st=>[st.adm,st.name,`${((st.P/st.total)*100).toFixed(1)}%`,`PKR ${st.outstanding}`,st.status]),
      styles:{fontSize:10}, margin:{left:margin,right:margin} });
    doc.addPage('landscape'); y=60;
    doc.setFontSize(18).text(`${school} – Attendance Detailed Report`,margin,y); y+=30;
    doc.setFontSize(12).text(`Fines & Eligibility Details for ${analyticsFrom} – ${analyticsTo}`,margin,y); y+=25;

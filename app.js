...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  btnSaveAtt.onclick = async e => {
    ...
    filteredStudents().forEach(s => {
      ...
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${s.name}</tg><td>${status}</td><td><button class="send-btn">Send</button></td>`;
  ...
  btnDownloadAnalytics.onclick = e => {
    ...
    doc.autoTable({
      ...
      body: Array.from(divAnalyticsTable.querySelectorAll('tbody tr')).map(r => Array.from(r.querySelectorAll('td')).map(c => c.textContent)),
      startY: 44
    });

...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('shareRegistration').onclick = () => {
    ...
    const lines = students
      ...
      .map(s => {
        ...
        const out = totalFine - paid, pct = total? stats.P/total*100 : 0;
  ...
  $('addStudent').onclick=async e=>{ e.preventDefault();
    ...
    if(!n||!p||!c||!o||!a){alert('All fields required');return;} if(!/^\d{7,15}$/.test(c)){alert('Contact digits');return;}
  ...
  function downloadAnalyticsHandler() {
    // call the click handler defined above
  ...
  loadA.onclick=()=>{
    ...
    if(atype.value==='student'){const q=asearch.value.trim().toLowerCase();pool=pool.filter(s=>s.adm===q||s.name.toLowerCase().includes(q));}
  ...
  $('applyAnalyticsFilter').onclick=()=>{ analyticsFilterOptions=Array.from(document.querySelectorAll('#analyticsFilterForm input[type="checkbox"]:checked')).map(cb=>cb.value);
    ...
    hide($('analyticsFilterModal')); if(!lastAnalyticsStats.length){alert('Load first');}else renderAnalytics(lastAnalyticsStats,lastAnalyticsRange.from,lastAnalyticsRange.to);
  };

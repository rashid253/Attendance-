...
window.addEventListener('DOMContentLoaded', () => {
  ...
  downloadAnalyticsBtn.onclick = ev => {
    ...
    // table
    const tableEl = analyticsContainer.querySelector('table');
    doc.autoTable({
      html: tableEl,
      startY:75,
  ...
  downloadRegBtn2.onclick = e => {
    ...
    // full register
    doc.autoTable({ html: document.getElementById('registerTable'), startY: 75, styles: { fontSize: 8 } });
    // summary register
    const summaryEl = document.querySelector('#registerSummarySection table');
    doc.autoTable({ html: summaryEl, startY: doc.lastAutoTable.finalY + 10, styles: { fontSize: 8 } });

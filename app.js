...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('addStudent').onclick = async e => {
    e.preventDefault();
    const data = ['studentName','parentName','parentContact','parentOccupation','parentAddress']
      .map(id => $(id).value.trim());
    if (data.some(v => !v)) return alert('All fields required');
    if (!/^\d{7,15}$/.test(data[2])) return alert('Contact must be 7–15 digits');
    const [name,parent,contact,occupation,address] = data;
    const cls = $('teacherClassSelect').value;
    const sec = $('teacherSectionSelect').value;
  ...
  $('loadAnalytics').onclick = () => {
    // ...similar to previous analytics implementation...

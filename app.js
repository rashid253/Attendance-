...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // --- 7. STUDENT REGISTRATION & EDIT/DELETE/SAVE ---
  ...
  $('downloadRegistrationPDF').onclick = () => {
    ...
    doc.setFontSize(12); doc.text(`Class ${cl} Section ${sec}`,14,24);
  ...
  $('loadAttendance').onclick = () => {
    ...
    show(ab,$('saveAttendance')); hide($('resetAttendance'),$('downloadAttendancePDF'),$('shareAttendanceSummary'),$('attendanceSummary'));
  };
  ...
  // --- download/share functions for Attendance Summary ---
  ...
  // --- Analytics: Load, Render, Download, Share ---
  ...
  $('downloadAnalytics').onclick = () => {
    ...
    if (analyticsDownload === 'combined') {
      ...
      doc.setFontSize(12);
      doc.text(`Period: ${analyticsRange.from} to ${analyticsRange.to}`, 14, 24);
      const body = filtered.map((st,i) => [
        i+1, st.adm, st.name,
        st.P, st.A, st.Lt, st.HD, st.L,
        st.total, `${st.pct.toFixed(1)}%`,
        `PKR ${st.outstanding.toFixed(0)}`,
        st.status
      ]);
      doc.autoTable({
        startY: 32,
        head: [['#','Adm#','Name','P','A','Lt','HD','L','Total','%','Outstanding','Status']],
        body, styles:{ fontSize:10 }
      });
      ...
      filtered.forEach(st => {
        ...
        const rows = [
          ...
          ['Half-Day', st.HD], ['Leave', st.L],
          ['Total', st.total], ['% Present', `${st.pct.toFixed(1)}%`],
  ...
  $('shareAnalytics').onclick = () => {
    const lines = analyticsStats.map((st,i) =>

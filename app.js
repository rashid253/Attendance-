...
window.addEventListener('DOMContentLoaded', () => {
  ...
  function hideAllAnalytics(){
    [analyticsDate,analyticsMonth,semesterStart,semesterEnd,yearStart,instructionsEl,analyticsContainer,graphsEl,analyticsActionsEl,resetAnalyticsBtn].forEach(el=>el.classList.add('hidden'));
  }
  ...
  const regMonthIn=$('registerMonth'),
        loadRegBtn=$('loadRegister'),
        changeRegBtn=$('changeRegister'),
        regTableWrapper=$('registerTableWrapper'),
        regTable=$('registerTable'),
        regBody=$('registerBody'),
        regSummarySection=$('registerSummarySection'),
        regSummaryBody=$('registerSummaryBody'),
        shareRegBtn2=$('shareRegister'),
        downloadRegBtn2=$('downloadRegisterPDF');
  const headerRow=regTable.querySelector('thead tr');
  for(let d=1;d<=31;d++){ const th=document.createElement('th'); th.textContent=d; headerRow.append(th); }
  loadRegBtn.onclick=e=>{
    ...
    stats.forEach(st=>{
      for(let d=1;d<=dim;d++){
        const key=`${regMonthIn.value}-${String(d).padStart(2,'0')}`;
        const rec=(JSON.parse(localStorage.getItem('attendanceData')||'{}'))[key]||{};

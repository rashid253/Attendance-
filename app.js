...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  $('saveSettings').onclick = async () => {
    ...
    $('settingsCard').innerHTML = `
      <p>Fine – Absent: PKR ${fineA}</p>
      <p>Fine – Late: PKR ${fineLt}</p>
      <p>Fine – Leave: PKR ${fineL}</p>
      <p>Fine – Half-Day: PKR ${fineHD}</p>
  ...
  function resetViews() {
    [
      ...
      $('shareAttendanceSummary'), $('instructions'), $('analyticsContainer'),
      $('graphs'), $('analyticsActions'), $('registerTableWrapper'),
      $('changeRegister'), $('saveRegister'), $('downloadRegister'),
      $('shareRegister')
    ].forEach(el => hide(el));
  }
  ...
  $('downloadRegistrationPDF').onclick = async () => {
    ...
    if (navigator.canShare && navigator.canShare({ files: [new File([blob], 'Registration.pdf', { type: 'application/pdf' })] })) {
  ...
  // --- 10. STUDENT REGISTRATION ACTIONS ---
$('editSelected').onclick = () => {
  document.querySelectorAll('.sel:checked').forEach(cb => {
    const tr = cb.closest('tr');
    // enable inline editing on Name, Parent, Contact, Occupation, Address
    [2, 4, 5, 6, 7].forEach(i => {
...
$('doneEditing').onclick = async () => {
  document.querySelectorAll('.sel:checked').forEach(cb => {
    ...
    // inputs correspond to columns 2,4,5,6,7 in order
    const [nameIn, parentIn, contactIn, occIn, addrIn] = inputs;
    Object.assign(students[idx], {
      name: nameIn.value,
      parent: parentIn.value,
      contact: contactIn.value,
      occupation: occIn.value,
  ...
  table.innerHTML = `
    <thead>
      <tr>
        ...
        <th>Present</th><th>Late</th><th>Leave</th><th>Absent</th><th>Half‑Day</th>
      </tr>
    </thead>
    <tbody></tbody>
  `;
      ...
      row.innerHTML = `
        ...
        ${['P','Lt','L','A','HD'].map(code => `
          <td><input type="radio" name="att_${s.adm}" value="${code}"
            ${prev===code?'checked':''} class="status-select" data-adm="${s.adm}"></td>
        `).join('')}
      `;
...
$('saveAttendance').onclick = async () => {
  const date = $('dateInput').value;
  attendanceData[date] = {};
  document.querySelectorAll('.status-select').forEach(radio => {
    if (radio.checked) attendanceData[date][radio.dataset.adm] = radio.value;
  });
  await save('attendanceData', attendanceData);
  alert('Attendance saved');
};

$('resetAttendance').onclick = () => renderAttendance($('dateInput').value);

$('shareAttendanceSummary').onclick = async () => {
  const doc = new jsPDF();
  doc.autoTable({ html: '#attendanceTable' });
  const blob = doc.output('blob');
  if (navigator.canShare({ files: [new File([blob], 'Attendance.pdf', { type: 'application/pdf' })] })) {
...
$('loadAnalytics').onclick = () => {
  // assume computeAnalytics fills analyticsTable, charts
  computeAnalytics();
...
$('loadRegister').onclick = () => {
  const mon = $('registerMonth').value;
  if (!mon) return alert('Select a month');
  loadRegister(mon);
...
$('saveRegister').onclick = async () => {
  const mon = $('registerMonth').value;
  await save(`register_${mon}`, /* your register data object */);
  alert('Register saved');
};

$('shareRegister').onclick = async () => {
  const doc = new jsPDF();
  doc.autoTable({ html: '#registerTable' });
  const blob = doc.output('blob');
  if (navigator.canShare({ files: [new File([blob], 'MonthlyRegister.pdf', { type: 'application/pdf' })] })) {

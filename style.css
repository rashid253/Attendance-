...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // 3. ATTENDANCE MARKING (unchanged styles & logic restored)
  ...
  // **FIXED**: hide graphs on Change Period
  ...
  loadAnalyticsBtn.onclick = ev => {
    ev.preventDefault();
    /* ... existing analytics computing code ... */
  ...
  loadRegBtn.onclick = e => {
    ...
    stats.forEach(st => {
      for (let d=1; d<=dim; d++) {
        ...
        const code = rec[s.students?.find(x=>x.name===st.name)?.roll]||'A';
        st[code]++; st.total++;
      }
  ...
  // **UPDATED**: PDF download with styling

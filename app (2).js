...
window.addEventListener('DOMContentLoaded', () => {
  ...
  loadAttBtn.onclick = ev => {
    ...
    students.forEach(s=>{
      ...
      ['P','A','Lt','HD','L'].forEach(code=>{
        ...
        b.onclick=e2=>{e2.preventDefault(); btns.querySelectorAll('.att-btn').forEach(x=>{x.style.background='';x.style.color='var(--dark)';}); b.style.background=colors[code];b.style.color='#fff';};
        btns.append(b);
      });
  ...
  loadAnalyticsBtn.onclick = e => {
    ...
    if (analyticsType.value === 'date') {
      ...
      if (!semesterStart.value || !semesterEnd.value) return alert('Pick a range');
  ...
  shareAnalyticsBtn.onclick = e => {
    ...
    const rows = Array.from(document.querySelectorAll('#analyticsContainer tbody tr')).map(r => {
      const vals = Array.from(r.querySelectorAll('td')).map(td => td.textContent);
      return `${vals[0]} P:${vals[1]} A:${vals[2]} Lt:${vals[3]} HD:${vals[4]} L:${vals[5]} %:${vals[7]}`;
    }).join('\n');
    window.open(`https://wa.me/?text=${encodeURIComponent(hdr + '\n\n' + rows)}`, '_blank');
  };
  ...
  const registerMonth = $('registerMonth'),
        ...
        downloadRegisterPDFBtn = $('downloadRegisterPDF');
  ...
  loadRegisterBtn.onclick = () => {
    ...
    students.forEach((s,i)=>{
      html += '<tr>';
      html += `<td>${i+1}</td><td>${s.adm}</td><td>${s.name}</td>`;
      for (let d=1; d<=daysInMonth; d++) {
        const dd=String(d).padStart(2,'0');
        const key=`${year}-${String(month).padStart(2,'0')}-${dd}`;
  ...
  shareRegisterBtn.onclick = ()=>{
    const period = registerMonth.value;
    const hdr = `Register Month: ${period}`;
  ...
  downloadRegisterPDFBtn.onclick = ()=>{
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF('p','pt','a4');
    doc.autoTable({ html:'#registerTableWrapper table', startY:40, margin:{left:40,right:40},styles:{fontSize:8} });
    doc.addPage();
    doc.autoTable({ html:'#registerSummary table', startY:40, margin:{left:40,right:40},styles:{fontSize:10} });
    const y = doc.lastAutoTable.finalY + 20;
    doc.addImage(regBarChart.toBase64Image(),'PNG',40,y,200,100);
    doc.addImage(regPieChart.toBase64Image(),'PNG',260,y,200,100);
    doc.save('traditional_register.pdf');
  };
